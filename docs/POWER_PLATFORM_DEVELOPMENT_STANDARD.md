...
**手順**:
```
1. Power Automate (https://make.powerautomate.com) でソリューション内のフローを開く
2. フロー編集画面に入ると、接続の承認ダイアログが表示される
   - Microsoft Dataverse と Office 365 Outlook の接続を確認
   - 各接続の横の ✅ を確認して「続行」をクリック
3. 保存するだけで接続参照が自動的に作成・紐付けされる
```

> **ポイント**: 各アクションを個別に変更する必要はない。フロー編集画面を開いて接続を承認し保存するだけで、ソリューション全体の接続参照が一括で作成される。
