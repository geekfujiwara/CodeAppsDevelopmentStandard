...
Dataverse テーブルの作成は **VS Code + GitHub Copilot（Awesome Copilot の Dataverse プラグイン）** を使って会話形式で行うことを**推奨**します。Power Platform 管理センター UI での手動作成よりもこちらを優先してください。

### 前提条件

1. VS Code に **Awesome Copilot** の以下のプラグインをインストール済みであること:
   - **Code Apps** プラグイン
   - **Dataverse** プラグイン
2. GitHub Copilot が有効化されていること（推奨モデル: **Claude Opus 4.6**）
...
GitHub Copilot でのテーブル作成が困難な場合は、Power Platform 管理センターまたは PAC CLI を代替手段として使用できます:

```bash
# PAC CLI でのソリューション作成
pac solution init --publisher-name "YourPublisher" --publisher-prefix "yp"

# Power Platform 管理センターでテーブルを手動作成
...
# テーブルの作成・管理は GitHub Copilot（Awesome Copilot Dataverse プラグイン）を優先
# 代替手段として Power Platform 管理センターまたは PAC CLI でも実施可能
```
