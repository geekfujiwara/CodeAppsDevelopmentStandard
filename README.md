...
- [クイックスタート](#クイックスタート)
- [環境準備プロンプト（1回実行・二段階不要）](#環境準備プロンプト1回実行二段階不要)
...
## 環境準備プロンプト（1回実行・二段階不要）

以下を GitHub Copilot / Claude Code にそのまま貼り付けると、Windows 端末の初期環境準備から **fork 済みリポジトリ** の clone / install / setup まで 1 回の依頼で実行できます。

```text
Windows 端末の開発環境準備を、フォーク済みリポジトリを使った新規テーマ開発の初期化まで含めて一括で実行してください（段階分けせず 1 回で完了させる）。
...
5. `https://github.com/geekfujiwara/CodeAppsDevelopmentStandard` を自分の GitHub アカウントに fork し、fork 先リポジトリを clone して `npm install` と `npm run setup` を順に実行する。
6. 最後に各ツールと preflight の動作確認を実施する。
...
- clone は空ディレクトリで実行する（`git clone https://github.com/<your-account>/CodeAppsDevelopmentStandard .`）。
- `git clone` / `npm install` / `npm run setup` は別行で順に実行し、`&&` で連結しない。
- `npm install` 後に `npm run setup` を実行し、preflight と Python bootstrap を完了させる。
