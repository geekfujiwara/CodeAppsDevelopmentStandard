...
新規テーマ開発は **本リポジトリをリモートに fork せず、ローカルに直接 clone してローカル開発** する方針です。GitHub 上には fork を作らず、手元の作業ディレクトリだけでテーマごとの開発を進めます。リモートに push したい場合は、開発開始後に必要に応じて自分のリモートリポジトリを作成して切り替えてください。

```bash
# 本リポジトリをローカルにクローン（fork しない）
...
> [!TIP]
> ローカル開発に専念したい場合は、リモート `origin` をそのままにしておいて push しない運用で問題ありません。独自リモートに切り替える場合は `git remote set-url origin <your-repo-url>` を使ってください。上流の最新を取り込む場合は `git pull origin main` を実行します。
