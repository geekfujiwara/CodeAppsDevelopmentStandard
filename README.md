...
- 以下のコマンドを PowerShell で実行し、**CodeAppsStarter テンプレートをカレントフォルダにコピー**してローカルで開発を開始します。

```bash
# テンプレートをカレントフォルダにコピー（リモートは作らずローカルで開始）
npx degit geekfujiwara/CodeAppsStarter .
...
> [!TIP]
> まず `npx degit` でローカルにコピーして素早くプロトタイピングを始めるのがおすすめです。
...
`npx degit` でローカルから始めた場合は、後から以下でリモートを用意してチーム開発へ移行・統合できます。
...
> 最初から `gh repo create --template` で生成した場合は、この時点で既に専用の private リポジトリができているため、追加の手順は不要です。
