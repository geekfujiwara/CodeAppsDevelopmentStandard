...
新規テーマ開発は **GitHub に fork せず、ローカルだけで完結** させる方針です。GitHub 上で fork するとデフォルトでは public な fork リポジトリが自分のアカウント配下に作成され、テーマ固有の業務情報が意図せず外部公開されるリスクがあります。これを避けるため、本リポジトリは「匿名 clone → ローカルで開発」のローカル運用を推奨します。

```bash
# 本リポジトリをローカルにクローン（fork しない / GitHub アカウント不要）
git clone https://github.com/geekfujiwara/CodeAppsDevelopmentStandard.git .

# 上流追跡用に origin を upstream に改名し、origin への誤 push を防止
...
> [!IMPORTANT]
> `git remote set-url --push upstream DISABLE` により upstream への push が無効化されます。これでテーマ固有のコミットを誤って外部公開してしまう事故を防げます。

### バックアップや共有のために自分のリモートを用意する場合

完全に手元だけで保管する以外に、**private リポジトリ** を別途用意してそこにのみ push する運用にすると安全です。public な fork は作成しません。
