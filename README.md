...
Power Apps Code Apps, モデル駆動型アプリ, Generative page, Dataverse, Power Automate, Power Pages, Copilot Studio に加えて Azure サービス、さらには Cowork のプラグインを **VS Code + GitHub Copilot / Claude Code** で開発するための、実践的な AI ファースト開発標準リポジトリです。
...
- Power Platform / Azure 向け AI ファースト開発標準（`.github/skills/*/references/`）
...
- [Power Platform 環境要件](#power-platform-環境要件)
  - [環境準備プロンプト](#環境準備プロンプト)
- [クイックスタート](#クイックスタート)
...
> [!CAUTION]
> **デフォルト環境（Default）では開発しないでください。** デフォルト環境は組織全員が参照できる共有環境のため、テーブルやソリューションが意図せず他ユーザーに見えます。必ず専用の **Developer 環境** または **Sandbox 環境** を用意してください。
...
> [!NOTE]
> **VS Code と GitHub Copilot がまだない方はまず以下を実施してください（所要 5 分）**
>
> | ステップ | 操作 |
> |---|---|
> | 1. VS Code をインストール | [https://code.visualstudio.com/](https://code.visualstudio.com/) からダウンロード → インストール |
> | 2. GitHub Copilot 拡張をインストール | VS Code を開き、拡張機能 `Ctrl+Shift+X` → **"GitHub Copilot"** を検索 → インストール |
> | 3. GitHub アカウントでサインイン | 左下のアカウントアイコン → GitHub でサインイン（Copilot サブスクリプションが必要） |
> | 4. Copilot チャットを開く | `Ctrl+Alt+I`（Mac: `⌃⌘I`）または左サイドバーのチャットアイコンをクリック |
>
> チャットが開いたら、以下のプロンプトをそのまま貼り付けて送信してください。
