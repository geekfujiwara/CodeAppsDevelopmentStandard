...
> [!IMPORTANT]
> **このリポジトリは開発標準（スキル・リファレンス）の供給元であり、新規テーマの雛形ではありません。**
> 新しいテーマは [CodeAppsStarter](https://github.com/geekfujiwara/CodeAppsStarter) テンプレートから生成し、
> 本リポジトリの `.github/`（エージェント・スキル）を `npm run setup` で同期して取り込みます。
> 本リポジトリを直接 clone してテーマ開発を始めると、サンプル実装のデザインやデータソース接続が
> 新テーマに残ってしまうため、この方式は廃止しました。
...
- 以下のコマンドを PowerShell で実行し、**CodeAppsStarter テンプレートから新しいテーマのリポジトリを生成**します。

```bash
# テンプレートから新規リポジトリを生成して clone（private 推奨）
gh repo create <your-account>/<your-theme-repo> --template geekfujiwara/CodeAppsStarter --private --clone
cd <your-theme-repo>

# 依存導入 + 開発標準（.github/ のエージェント・スキル）の同期 + 環境チェック
npm install
npm run setup
```

> リモートを作らずローカルだけで試す場合は、clone の代わりに
> `npx degit geekfujiwara/CodeAppsStarter <フォルダ名>` でコピーしてから `npm install && npm run setup` を実行します。
...
**サンプル実装（`.github/skills/*/samples/`）は同期されません**。前テーマのデザインやデータソース接続が
新テーマに混入することはありません。
...
`gh repo create --template` で生成した場合は、この時点で既に専用の private リポジトリができています。追加の手順は不要です。

`npx degit` でローカルコピーから始めた場合は、後から以下でリモートを用意できます。
...
git init && git add -A && git commit -m "init from CodeAppsStarter"
...
git merge は不要です。テーマのリポジトリで同期コマンドを再実行するだけで、最新の標準（`.github/` のエージェント・スキル）に追従できます。
...
> [!TIP]
> 同期対象は `.github/` と共有ファイルのみで、テーマ固有のコード（`src/` 等）には触れません。
