...
`npm run setup` で **環境事前チェック (preflight) + Python bootstrap** をまとめて実行し、Node.js / npm / Python（`python` or `py -3`）/ pip / `npx power-apps` / `pac` を確認します。
...
以下を GitHub Copilot / Claude Code にそのまま貼り付けると、Windows 端末の初期環境準備から clone / install / setup まで 1 回の依頼で実行できます。

```text
Windows 端末の開発環境準備を、リポジトリ初期化まで含めて一括で実行してください（段階分けせず 1 回で完了させる）。
...
5. リポジトリを clone し、`npm install` と `npm run setup` を順に実行する。
...
- clone は空ディレクトリで実行する（`git clone https://github.com/geekfujiwara/CodeAppsDevelopmentStandard .`）。
