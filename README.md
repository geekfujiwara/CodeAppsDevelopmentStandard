...
新規テーマ開発は **自分のアカウントに fork してからローカルに clone してローカル開発** する方針です。`gh repo fork --clone` を使って fork とローカル clone を 1 コマンドで実施し、手元の作業ディレクトリでテーマごとの開発を進めます。fork 元（upstream）は自動で登録されるため、`git fetch upstream` で標準更新を取り込めます。

```bash
# fork + ローカル clone を 1 コマンドで実施（カレントディレクトリに clone）
gh repo fork geekfujiwara/CodeAppsDevelopmentStandard --clone=true -- .
npm install
npm run setup
```

> [!NOTE]
> `.` へ clone するため、空ディレクトリで実行してください。既存ファイルがある場所で実行すると上書きリスクがあります。fork 済みの場合は `gh repo fork` が既存 fork を検出して clone のみ実行します。

### 上流の開発標準更新を取り込む

`gh repo fork --clone` で clone した場合、`upstream` は自動登録されています（未登録なら `git remote add upstream https://github.com/geekfujiwara/CodeAppsDevelopmentStandard.git`）。
