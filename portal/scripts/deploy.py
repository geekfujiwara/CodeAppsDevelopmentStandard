...
SITE_NAME = os.environ.get("PP_SITE_NAME", "IncidentPortal")
...
def ask_subdomain():
    ...
    if SUBDOMAIN:
        return
    print("\n[Pre] PP_SUBDOMAIN が .env に未設定です。サイトを検索します...")
    ...
    if not sites:
        print("  環境にサイトが見つかりません。PP_SUBDOMAIN を .env に手動設定してください。")
        sys.exit(1)
    print("  検出されたサイト:")
    ...
    while True:
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(sites):
            ...
            with open(env_path, "r", encoding="utf-8") as f:
                content = f.read()
            if "PP_SUBDOMAIN=" in content:
                import re
                content = re.sub(r"PP_SUBDOMAIN=.*", f"PP_SUBDOMAIN={SUBDOMAIN}", content)
            else:
                content += f"\nPP_SUBDOMAIN={SUBDOMAIN}\n"
            if "PP_SITE_NAME=" not in content:
                content += f"PP_SITE_NAME={selected.get('name', SITE_NAME)}\n"
            with open(env_path, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"  .env に PP_SUBDOMAIN={SUBDOMAIN} を保存しました")
            return
