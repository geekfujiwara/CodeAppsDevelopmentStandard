...
25. **`optInUseLatestModels` は明示的に `False` を設定**。`True` にすると UI で選んだ基盤モデル（Claude 等）が GPT に強制変更される。既存 config に True が残っていても False で上書きする
...
1. エージェントの目的・役割に合ったアイコン画像を 3〜4 パターン生成
2. 各パターンに説明を付けて提示
3. ユーザーに選択してもらう
4. 選択されたアイコンを Bot 作成時に Copilot Studio UI で設定するよう案内

**Step B: 構築・デプロイ**
1. Copilot Studio UI でエージェント作成（API では作成不可）— ユーザーにはソリューションの**表示名とスキーマ名の両方**を伝える。選択されたアイコンの設定も案内
2. カスタムトピック全削除
3. 生成オーケストレーション有効化（configuration ディープマージ必須、optInUseLatestModels: False）
4. 指示（Instructions）+ 推奨プロンプト設定（GPT コンポーネントの conversationStarters）
5. 会話の開始のクイック返信設定（ConversationStart トピックの quickReplies）
6. エージェント公開（PvaPublish）
7. 説明の設定（publish 後に botcomponents.description を PATCH）
8. Teams / Copilot チャネル公開設定（applicationmanifestinformation をPATCH）
9. チャネル公開実行（channels 設定 + 最終 PvaPublish）
10. ★ ナレッジ追加（ユーザーに UI 操作を依頼）
11. ★ MCP Server 追加（ユーザーに UI 操作を依頼）
