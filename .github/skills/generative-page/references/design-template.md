...
### Pattern E: 拠点マップダッシュボード

**向いている場面**: 拠点管理・設備分布・顧客訪問計画・現地対応・海外拠点の一覧

地図は **Google Maps iframe 埋め込み**で実装する。URL レシピ・`sandbox` の必須トークン・ホスト別 CSP は
[地図埋め込みパターン](../../standard/references/map-embed-pattern.md) を参照
（**Generative Pages は CSP 設定なしで埋め込める**。Code Apps とは前提が異なる）。
...
| Tier | コンポーネント | 説明 |
|------|---------------|------|
| 1 | 地図 iframe | 選択拠点の座標／住所から埋め込み URL を組み立てて表示 |
| 1 | 拠点一覧 | クリックで選択 → 地図の `src` を差し替え |
| 1 | 詳細パネル | 選択拠点の属性・KPI を表示 |
| 1 | 未選択時のプレースホルダー | 座標が無い場合も含めて空状態を明示 |
| 2 | 地域フィルタ | Choice 列や国・都道府県で一覧を絞り込む |
| 2 | KPI カード連動 | 選択拠点の KPI を上部に表示 |
| 2 | ルート表示 | `saddr`/`daddr` で出発地→目的地の経路を表示 |
| 3 | ズーム切替 | `z` パラメータを 3（広域）〜18（詳細）で切替 |
...
- 地図は **1 枚だけ**描画し、選択に応じて `src` を差し替える（拠点ごとに iframe を並べない）
- 埋め込み URL は `https://maps.google.com/maps?q={query}&z=15&hl=ja&output=embed`。`query` は必ず `encodeURIComponent` を通す
- 座標は `isFinite()` で検証してから URL に連結する。Dataverse の値をそのまま連結しない
- `sandbox="allow-scripts allow-same-origin allow-popups"` を付ける。`allow-same-origin` を外すと Maps JS が `SecurityError` を投げる
- 海外拠点も `q` に住所・地名を渡すだけで表示される。地域別の実装分岐は不要
- `100vh` は禁止。地図の高さは固定 px（例: 420px）で確保する
...
| 進捗率                   | バー内進捗オーバーレイ              | D        |
| マイルストーン           | ◆マーカー                           | D        |
| 地理的分布                 | 拠点マップ (Google Maps iframe)   | E        |
| 稼働状況一覧             | ステータスボード                    | B,C      |
