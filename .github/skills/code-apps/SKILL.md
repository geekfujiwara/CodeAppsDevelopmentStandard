...
# `geek_*` 等の literal をハードコードしないこと（別環境で存在しないテーブルを参照する原因になる）。
node patch-nameutils.cjs   # 日本語パッチ（npm install 後に毎回必要）
source .env
npx power-apps add-data-source --api-id dataverse \
  --resource-name ${PUBLISHER_PREFIX}_{table_basename} \
...
PUBLISHER_PREFIX は必ず .env から読み込む。
ハードコードすると別環境（別 publisher prefix）のテーブルを誤って作成・操作する原因になる。
"""
import os
import sys
...
if not PREFIX:
    raise SystemExit(
        ...
        "プロジェクトの publisher prefix を .env に定義してください。"
    )

# テーブル論理名は (f"{PREFIX}_xxx", DisplayName, DisplayCollectionName) の形で定義
TABLES_EN = [
