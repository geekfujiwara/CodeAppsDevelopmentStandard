...
| [高度な実装パターン](references/advanced-patterns.md) | マルチ環境・i18n・テーマ・アニメーション・パフォーマンス最適化 |
| [コネクタ設定リファレンス](references/connector-reference.md) | 各コネクタの追加手順・設定方法・使用例 |

### Code Apps スターターテンプレート

プロジェクトの初期ファイル一式は [`template/`](template/) に格納されています。
新規プロジェクト開始時は `template/` 内のファイルをプロジェクトルートにコピーして使用します。
