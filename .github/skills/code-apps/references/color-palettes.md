...
7. design-pattern.md の設計フェーズ（画面一覧・コンポーネント選定）と合わせてユーザー承認を得る
```
...
### カテゴリ配色パレットは全パレット共通（汎用カテゴリ色）

`--chart-1`〜`--chart-5` は **単一系列の濃淡**（同一色相のグラデーション）で、棒グラフ 1 本・折れ線 1 本など
「1 つの系列」を美しく見せるためのもの。**種類を色で区別したい**用途（円グラフの内訳・ガント/カンバンの
カテゴリ・タグ・工場別など）には向かない。そこで **全パレット共通のカテゴリ配色**（バッジ同様、テーマを
切り替えても変えない記号色）を 1 セット定義し、アプリ全体で共通利用する。

```css
/* styles/index.pcss の :root（.dark と共通・上書きしない）。彩度/明度を揃え、黒・グレーは含めない */
--cat-1: #e60012;  /* red    */
--cat-2: #2563eb;  /* blue   */
--cat-3: #f59e0b;  /* amber  */
--cat-4: #16a34a;  /* green  */
...
これを **唯一の出所（single source）** とし、JS からは 1 つのユーティリティに集約して再利用する
（ページごとに `PIE_COLORS` のような色配列を複製しない — 複製すると後で低彩度色を除去しても反映漏れが起きる）:

```ts
// src/lib/category-color.ts — カテゴリ色の共通ユーティリティ（唯一の出所）
export const PALETTE = [
  "#e60012", "#2563eb", "#f59e0b", "#16a34a",
  "#8b5cf6", "#0ea5e9", "#db2777", "#ea580c",
] // 上記 --cat-1〜8 と一致させる

/** 自由入力カテゴリ（商品カテゴリ・工場名等）を一貫した色にハッシュマッピング */
...
| 用途 | 使うもの |
|---|---|
| 単一系列の棒・折れ線・面（1 系列の濃淡） | `var(--chart-1)`〜`var(--chart-5)`（テーマ追従） |
| 内訳円グラフ・積み上げなど **系列を区別** | `PALETTE[i % PALETTE.length]` を Recharts `Cell fill=` に渡す |
| 自由入力カテゴリ（工場・商品カテゴリ等） | `categoryColor(値)` |
| 単発のアクセント（1 色だけ強調） | `var(--primary)`（テーマのブランド色） |

> Recharts の `Pie`/`Bar` で系列を色分けする場合も、独自配列を作らず `PALETTE` を import して
> `{data.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}` のように共通化する。
...
（`--badge-*` と `--cat-1`〜`--cat-8` は**共通**なのでパレット切替では変更しない。）
...
- [ ] カテゴリ配色（`--cat-1`〜`--cat-8` / `category-color.ts` の `PALETTE`）は共通のまま。ページ側で色配列を複製していない
