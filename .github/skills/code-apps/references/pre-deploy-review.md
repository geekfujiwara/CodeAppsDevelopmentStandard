...
### 7. ナビ ↔ ルーター整合性チェック（★テンプレート残骸防止）

`config.ts` のナビゲーションパスと `router.tsx` のルートパスが一致しているか確認する。
**テンプレートのデモメニューが残ったままデプロイされる問題を防ぐ**ために最も重要なチェック。

このチェックは `npm run predeploy`（`pre-deploy-check.mjs`）で自動実行される。

**チェック内容:**

| 不整合 | 意味 | 対処 |
|---|---|---|
| ナビにあるがルーターに無い | 孤立メニュー（クリックすると404） | router.tsx にルート追加 or config.ts から削除 |
| ルーターにあるがナビに無い | 隠しページ（ユーザーが到達できない） | config.ts にナビ追加 or router.tsx から削除 |

**よくある原因:**

```
❌ テンプレートの config.ts に「顧客」「商談」「テリトリー」等のデモメニューが残っている
   → テーマに無関係なナビを全て削除する

❌ 新ページを追加したが config.ts のナビに入れ忘れた
   → ページ追加時は必ず 3ファイル同時更新（pages/ + router.tsx + config.ts）
```
