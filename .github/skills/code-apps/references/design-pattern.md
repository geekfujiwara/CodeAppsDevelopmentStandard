...
## 設計フェーズ（ユーザー承認必須・承認ゲート）

**このスキルで設計した内容は、ユーザーに提示して承認を得てから実装に進む。承認前に実装コードを書かない。**
...
| **適用デザインパターン** | [デザインパターンカタログ](architecture-patterns.md) の選定マトリクスから選んだアーキテクチャパターン（レイヤード構成・サービスレイヤー・React Query フック・楽観的更新・useConfirm 等）を対象・理由付きで列挙 |
...
```
フロー: code-apps（design-system + architecture-patterns）で設計 → ユーザー承認 → code-apps で実装
       実装中に設計から逸脱する変更が必要になったら、差分を再提示して再承認 → 続行
```

> **承認ゲート**: 承認された設計が実装の唯一の基準。画面の追加/削除・適用パターンの変更・
> テーブルスキーマ変更が必要になった場合は、承認済み設計との差分を提示して再承認を得てから実装する。
>
> **承認の記録**: 承認の証跡を残す場合は Issue テンプレート
> [`design-approval.yml`](../../../ISSUE_TEMPLATE/design-approval.yml)（「Code Apps 設計承認」）で
> 設計内容と承認を Issue に記録する。再承認は同じ Issue に差分をコメントして行う。
