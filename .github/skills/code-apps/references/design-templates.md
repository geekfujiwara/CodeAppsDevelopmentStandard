...
## ページ内パターン（レイアウトと組み合わせて使う）

レイアウト（アプリシェル）とは別に、**ページ内部で繰り返し使う画面構成パターン**がある。
以下はどのレイアウトとも組み合わせ可能。

### パイプラインダッシュボード

Sidebar レイアウトの中に ReactFlow パイプライン（リード→商談フロー）と組織図を **Tabs で切り替え** て表示する構成。
フィルタ（リードソース・キャンペーン・会計期間）をヘッダー直下に配置し、KPI サマリーカードの下にフロー図を描画する。

→ 実装詳細: [ReactFlow パターン §5](reactflow-patterns.md#パターン-5-セールスパイプラインリード--商談フロー可視化)

### 詳細画面サマリーヘッダー + ステージ矢羽

Card の上半分に主要 KPI（取引先・日付・金額・確度）を `grid grid-cols-4` で表示し、
`border-t` の下に `StagePath`（Salesforce 風シェブロン矢羽）を配置。矢羽クリックでステージ即時変更。

→ 実装詳細: [ステージ矢羽パターン — 詳細画面の統合パターン](stage-path-pattern.md#詳細画面の統合パターンサマリーヘッダー--矢羽)
