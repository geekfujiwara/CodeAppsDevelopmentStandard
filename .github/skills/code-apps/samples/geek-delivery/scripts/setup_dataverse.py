"""
Dataverse テーブル構築: {PREFIX}_vehicle / {PREFIX}_delivery_route / {PREFIX}_stop
=========================================
配送管理ポータル（物流・運輸）アプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_vehicle", "display": "Vehicle", "plural": "Vehicles", "description": "車両マスタ",
        "columns": [
            {"logical": f"{PREFIX}_model",          "type": "String",   "display": "Model",  "maxLength": 200},
            {"logical": f"{PREFIX}_driver",         "type": "String",   "display": "Driver", "maxLength": 200},
            {"logical": f"{PREFIX}_status",         "type": "Picklist", "display": "Status",
             "options": [(100000000,"Available"),(100000001,"On Route"),(100000002,"Maintenance")]},
            {"logical": f"{PREFIX}_inspection_due", "type": "DateTime", "display": "Inspection Due", "format": "DateOnly"},
        ],
    },
    {
        "logical": f"{PREFIX}_delivery_route", "display": "Delivery Route", "plural": "Delivery Routes", "description": "配送便",
        "columns": [
            {"logical": f"{PREFIX}_vehicle_ref", "type": "String",   "display": "Vehicle Ref", "maxLength": 100},
            {"logical": f"{PREFIX}_driver",      "type": "String",   "display": "Driver",      "maxLength": 200},
            {"logical": f"{PREFIX}_status",      "type": "Picklist", "display": "Status",
             "options": [(100000000,"Planned"),(100000001,"On Route"),(100000002,"Completed")]},
            {"logical": f"{PREFIX}_route_date",  "type": "DateTime", "display": "Route Date",  "format": "DateOnly"},
            {"logical": f"{PREFIX}_notes",       "type": "Memo",     "display": "Notes",       "maxLength": 4000},
        ],
    },
    {
        "logical": f"{PREFIX}_stop", "display": "Stop", "plural": "Stops", "description": "配送先",
        "columns": [
            {"logical": f"{PREFIX}_route_ref",    "type": "String",   "display": "Route Ref",    "maxLength": 100},
            {"logical": f"{PREFIX}_seq",          "type": "Integer",  "display": "Sequence",     "minValue": 0, "maxValue": 100000},
            {"logical": f"{PREFIX}_area",         "type": "String",   "display": "Area",         "maxLength": 300},
            {"logical": f"{PREFIX}_planned_time", "type": "String",   "display": "Planned Time", "maxLength": 20},
            {"logical": f"{PREFIX}_status",       "type": "Picklist", "display": "Status",
             "options": [(100000000,"Pending"),(100000001,"Delivered"),(100000002,"Absent"),(100000003,"Returned")]},
...
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_vehicle",        "車両",   "車両一覧"),
    (f"{PREFIX}_delivery_route", "配送便", "配送便一覧"),
    (f"{PREFIX}_stop",           "配送先", "配送先一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_vehicle", f"{PREFIX}_name",           "車両名"),
    (f"{PREFIX}_vehicle", f"{PREFIX}_model",          "車種"),
    (f"{PREFIX}_vehicle", f"{PREFIX}_driver",         "担当ドライバー"),
    (f"{PREFIX}_vehicle", f"{PREFIX}_status",         "ステータス"),
    (f"{PREFIX}_vehicle", f"{PREFIX}_inspection_due", "点検期限"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_name",        "便名"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_vehicle_ref", "車両ID"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_driver",      "ドライバー"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_status",      "ステータス"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_route_date",  "運行日"),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_notes",       "備考"),
    (f"{PREFIX}_stop", f"{PREFIX}_name",         "配送先名"),
    (f"{PREFIX}_stop", f"{PREFIX}_route_ref",    "配送便ID"),
    (f"{PREFIX}_stop", f"{PREFIX}_seq",          "順序"),
    (f"{PREFIX}_stop", f"{PREFIX}_area",         "エリア・住所"),
    (f"{PREFIX}_stop", f"{PREFIX}_planned_time", "予定時刻"),
    (f"{PREFIX}_stop", f"{PREFIX}_status",       "配達ステータス"),
    (f"{PREFIX}_stop", f"{PREFIX}_remark",       "備考"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_vehicle",        f"{PREFIX}_status", [(100000000,"稼働可"),(100000001,"運行中"),(100000002,"整備中")]),
    (f"{PREFIX}_delivery_route", f"{PREFIX}_status", [(100000000,"計画"),(100000001,"運行中"),(100000002,"完了")]),
    (f"{PREFIX}_stop",           f"{PREFIX}_status", [(100000000,"未配達"),(100000001,"配達完了"),(100000002,"不在"),(100000003,"持ち戻り")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-delivery 配送管理ポータル"); print("="*60)
