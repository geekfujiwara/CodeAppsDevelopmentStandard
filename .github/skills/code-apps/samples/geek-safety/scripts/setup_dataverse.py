"""
Dataverse テーブル構築: {PREFIX}_incident / {PREFIX}_corrective_action
=========================================
安全衛生ポータル（ヒヤリハット報告）アプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_incident", "display": "Incident", "plural": "Incidents", "description": "ヒヤリハット・事故報告",
        "columns": [
            {"logical": f"{PREFIX}_site",          "type": "String",   "display": "Site",          "maxLength": 200},
            ...
            {"logical": f"{PREFIX}_severity",      "type": "Picklist", "display": "Severity",
             "options": [(100000000,"Near Miss"),(100000001,"Minor"),(100000002,"Serious"),(100000003,"Critical")]},
            {"logical": f"{PREFIX}_status",        "type": "Picklist", "display": "Status",
             "options": [(100000000,"Reported"),(100000001,"In Progress"),(100000002,"Resolved")]},
            {"logical": f"{PREFIX}_reporter",      "type": "String",   "display": "Reporter",      "maxLength": 200},
            {"logical": f"{PREFIX}_occurred_date", "type": "DateTime", "display": "Occurred Date", "format": "DateOnly"},
            {"logical": f"{PREFIX}_description",   "type": "Memo",     "display": "Description",   "maxLength": 4000},
            {"logical": f"{PREFIX}_cause",         "type": "Memo",     "display": "Cause",         "maxLength": 4000},
            {"logical": f"{PREFIX}_notes",         "type": "Memo",     "display": "Notes",         "maxLength": 4000},
        ],
    },
    {
        "logical": f"{PREFIX}_corrective_action", "display": "Corrective Action", "plural": "Corrective Actions", "description": "是正措置",
        "columns": [
            {"logical": f"{PREFIX}_incident_ref", "type": "String",   "display": "Incident Ref", "maxLength": 100},
            {"logical": f"{PREFIX}_assignee",     "type": "String",   "display": "Assignee",     "maxLength": 200},
            {"logical": f"{PREFIX}_status",       "type": "Picklist", "display": "Status",
             "options": [(100000000,"Not Started"),(100000001,"In Progress"),(100000002,"Completed")]},
...
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_incident",          "ヒヤリハット報告", "ヒヤリハット報告一覧"),
    (f"{PREFIX}_corrective_action", "是正措置",         "是正措置一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_incident", f"{PREFIX}_name",          "件名"),
    (f"{PREFIX}_incident", f"{PREFIX}_site",          "拠点・場所"),
    (f"{PREFIX}_incident", f"{PREFIX}_category",      "種別"),
    (f"{PREFIX}_incident", f"{PREFIX}_severity",      "重大度"),
    (f"{PREFIX}_incident", f"{PREFIX}_status",        "ステータス"),
    (f"{PREFIX}_incident", f"{PREFIX}_reporter",      "報告者"),
    (f"{PREFIX}_incident", f"{PREFIX}_occurred_date", "発生日"),
    (f"{PREFIX}_incident", f"{PREFIX}_description",   "状況詳細"),
    (f"{PREFIX}_incident", f"{PREFIX}_cause",         "原因"),
    (f"{PREFIX}_incident", f"{PREFIX}_notes",         "備考"),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_name",         "措置内容"),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_incident_ref", "報告ID"),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_assignee",     "担当者"),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_status",       "ステータス"),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_due_date",     "期限"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_incident", f"{PREFIX}_severity", [(100000000,"ヒヤリハット"),(100000001,"軽微"),(100000002,"重大"),(100000003,"重大災害")]),
    (f"{PREFIX}_incident", f"{PREFIX}_status",   [(100000000,"報告済み"),(100000001,"対応中"),(100000002,"是正完了")]),
    (f"{PREFIX}_corrective_action", f"{PREFIX}_status", [(100000000,"未着手"),(100000001,"対応中"),(100000002,"完了")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-safety 安全衛生ポータル"); print("="*60)
