"""
Dataverse テーブル構築: {PREFIX}_site / {PREFIX}_punch_item
=========================================
竣工検査ポータル（建設業・是正指摘管理）アプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_site", "display": "Site", "plural": "Sites", "description": "現場マスタ",
        "columns": [
            {"logical": f"{PREFIX}_manager",         "type": "String",   "display": "Manager", "maxLength": 200},
            {"logical": f"{PREFIX}_status",          "type": "Picklist", "display": "Status",
             "options": [(100000000,"Under Construction"),(100000001,"Final Inspection"),(100000002,"Handed Over")]},
            {"logical": f"{PREFIX}_completion_date", "type": "DateTime", "display": "Completion Date", "format": "DateOnly"},
        ],
    },
    {
        "logical": f"{PREFIX}_punch_item", "display": "Punch Item", "plural": "Punch Items", "description": "指摘事項",
        "columns": [
            {"logical": f"{PREFIX}_site_ref",   "type": "String",   "display": "Site Ref",   "maxLength": 100},
            ...
            {"logical": f"{PREFIX}_contractor", "type": "String",   "display": "Contractor", "maxLength": 200},
            {"logical": f"{PREFIX}_status",     "type": "Picklist", "display": "Status",
             "options": [(100000000,"Open"),(100000001,"In Progress"),(100000002,"Fixed"),(100000003,"Verified")]},
            {"logical": f"{PREFIX}_found_date", "type": "DateTime", "display": "Found Date", "format": "DateOnly"},
            {"logical": f"{PREFIX}_due_date",   "type": "DateTime", "display": "Due Date",   "format": "DateOnly"},
            {"logical": f"{PREFIX}_detail",     "type": "Memo",     "display": "Detail",     "maxLength": 4000},
        ],
    },
]
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_site",       "現場",     "現場一覧"),
    (f"{PREFIX}_punch_item", "指摘事項", "指摘事項一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_site", f"{PREFIX}_name",            "現場名"),
    (f"{PREFIX}_site", f"{PREFIX}_manager",         "現場代理人"),
    (f"{PREFIX}_site", f"{PREFIX}_status",          "ステータス"),
    (f"{PREFIX}_site", f"{PREFIX}_completion_date", "竣工予定日"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_name",       "指摘内容"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_site_ref",   "現場ID"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_location",   "場所"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_category",   "分類"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_contractor", "担当業者"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_status",     "ステータス"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_found_date", "指摘日"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_due_date",   "是正期限"),
    (f"{PREFIX}_punch_item", f"{PREFIX}_detail",     "詳細"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_site",       f"{PREFIX}_status", [(100000000,"施工中"),(100000001,"竣工検査"),(100000002,"引渡し済み")]),
    (f"{PREFIX}_punch_item", f"{PREFIX}_status", [(100000000,"指摘"),(100000001,"是正中"),(100000002,"是正済"),(100000003,"確認済")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-punchlist 竣工検査ポータル"); print("="*60)
