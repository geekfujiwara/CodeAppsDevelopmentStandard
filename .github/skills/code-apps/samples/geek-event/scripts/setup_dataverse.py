"""
Dataverse テーブル構築: {PREFIX}_event / {PREFIX}_registration
=========================================
イベント管理ポータルアプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_event", "display": "Event", "plural": "Events", "description": "イベント",
        "columns": [
            {"logical": f"{PREFIX}_category",    "type": "String",   "display": "Category",    "maxLength": 200},
            {"logical": f"{PREFIX}_venue",       "type": "String",   "display": "Venue",       "maxLength": 200},
            {"logical": f"{PREFIX}_organizer",   "type": "String",   "display": "Organizer",   "maxLength": 200},
            {"logical": f"{PREFIX}_status",      "type": "Picklist", "display": "Status",
             "options": [(100000000,"Open"),(100000001,"Full"),(100000002,"Closed"),(100000003,"Cancelled")]},
    ...
    {
        "logical": f"{PREFIX}_registration", "display": "Registration", "plural": "Registrations", "description": "参加登録",
        "columns": [
            {"logical": f"{PREFIX}_event_ref",       "type": "String",   "display": "Event Ref",       "maxLength": 100},
            {"logical": f"{PREFIX}_department",      "type": "String",   "display": "Department",      "maxLength": 200},
            {"logical": f"{PREFIX}_status",          "type": "Picklist", "display": "Status",
             "options": [(100000000,"Registered"),(100000001,"Attended"),(100000002,"Absent"),(100000003,"Cancelled")]},
            {"logical": f"{PREFIX}_registered_date", "type": "DateTime", "display": "Registered Date", "format": "DateOnly"},
        ],
    },
]
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_event",        "イベント", "イベント一覧"),
    (f"{PREFIX}_registration", "参加登録", "参加登録一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_event", f"{PREFIX}_name",        "イベント名"),
    (f"{PREFIX}_event", f"{PREFIX}_category",    "分野"),
    (f"{PREFIX}_event", f"{PREFIX}_venue",       "会場"),
    (f"{PREFIX}_event", f"{PREFIX}_organizer",   "主催者"),
    (f"{PREFIX}_event", f"{PREFIX}_status",      "ステータス"),
    (f"{PREFIX}_event", f"{PREFIX}_start_date",  "開催日"),
    (f"{PREFIX}_event", f"{PREFIX}_capacity",    "定員"),
    (f"{PREFIX}_event", f"{PREFIX}_description", "概要"),
    (f"{PREFIX}_registration", f"{PREFIX}_name",            "参加者名"),
    (f"{PREFIX}_registration", f"{PREFIX}_event_ref",       "イベントID"),
    (f"{PREFIX}_registration", f"{PREFIX}_department",      "部門"),
    (f"{PREFIX}_registration", f"{PREFIX}_status",          "ステータス"),
    (f"{PREFIX}_registration", f"{PREFIX}_registered_date", "登録日"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_event",        f"{PREFIX}_status", [(100000000,"募集中"),(100000001,"満席"),(100000002,"終了"),(100000003,"中止")]),
    (f"{PREFIX}_registration", f"{PREFIX}_status", [(100000000,"登録"),(100000001,"出席"),(100000002,"欠席"),(100000003,"キャンセル")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-event イベント管理ポータル"); print("="*60)
