"""
Dataverse テーブル構築: {PREFIX}_contract / {PREFIX}_counterparty
=========================================
契約台帳管理ポータルアプリ用テーブル。
"""
import os
...
TABLES = [
    {
        "logical": f"{PREFIX}_contract",
        "display": "Contract",
        "plural":  "Contracts",
        "description": "契約台帳",
        "columns": [
            {"logical": f"{PREFIX}_contract_number", "type": "String",   "display": "Contract Number", "maxLength": 100},
            {"logical": f"{PREFIX}_counterparty",    "type": "String",   "display": "Counterparty",    "maxLength": 200},
            {"logical": f"{PREFIX}_contract_type",   "type": "Picklist", "display": "Contract Type",
             "options": [(100000000,"Service Agreement"),(100000001,"Sales"),(100000002,"Lease"),(100000003,"Maintenance"),(100000004,"Other")]},
            ...
            {"logical": f"{PREFIX}_auto_renewal",    "type": "Picklist", "display": "Auto Renewal",
             "options": [(100000000,"Yes"),(100000001,"No")]},
            {"logical": f"{PREFIX}_status",          "type": "Picklist", "display": "Status",
             "options": [(100000000,"Active"),(100000001,"Expired"),(100000002,"Renewing"),(100000003,"Terminated"),(100000004,"Draft")]},
            {"logical": f"{PREFIX}_value",           "type": "Decimal",  "display": "Contract Value",  "precision": 2, "maxValue": 100000000000},
            {"logical": f"{PREFIX}_notes",           "type": "Memo",     "display": "Notes",           "maxLength": 4000},
        ],
    },
    {
        "logical": f"{PREFIX}_counterparty",
        "display": "Counterparty",
        "plural":  "Counterparties",
        "description": "取引先マスタ",
        "columns": [
            {"logical": f"{PREFIX}_company_type",  "type": "Picklist", "display": "Company Type",
             "options": [(100000000,"Customer"),(100000001,"Supplier"),(100000002,"Partner"),(100000003,"Other")]},
            {"logical": f"{PREFIX}_contact_name",  "type": "String",   "display": "Contact Name",  "maxLength": 200},
            {"logical": f"{PREFIX}_contact_email", "type": "String",   "display": "Contact Email", "maxLength": 200},
...
LOCALIZE_TABLES = [
    (f"{PREFIX}_contract",     "契約台帳", "契約台帳一覧"),
    (f"{PREFIX}_counterparty", "取引先",   "取引先一覧"),
]

LOCALIZE_COLUMNS = [
    (f"{PREFIX}_contract", f"{PREFIX}_name",            "件名"),
    (f"{PREFIX}_contract", f"{PREFIX}_contract_number", "契約番号"),
    (f"{PREFIX}_contract", f"{PREFIX}_counterparty",    "取引先"),
    (f"{PREFIX}_contract", f"{PREFIX}_contract_type",   "契約種別"),
    (f"{PREFIX}_contract", f"{PREFIX}_start_date",      "開始日"),
    (f"{PREFIX}_contract", f"{PREFIX}_end_date",        "終了日"),
    (f"{PREFIX}_contract", f"{PREFIX}_auto_renewal",    "自動更新"),
    (f"{PREFIX}_contract", f"{PREFIX}_status",          "ステータス"),
    (f"{PREFIX}_contract", f"{PREFIX}_value",           "契約金額"),
    (f"{PREFIX}_contract", f"{PREFIX}_notes",           "備考"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_name",          "取引先名"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_company_type",  "会社種別"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_contact_name",  "担当者名"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_contact_email", "メール"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_phone",         "電話"),
    (f"{PREFIX}_counterparty", f"{PREFIX}_notes",         "備考"),
]

LOCALIZE_OPTIONS = [
    (f"{PREFIX}_contract", f"{PREFIX}_contract_type", [
        (100000000,"業務委託"),(100000001,"売買"),(100000002,"賃貸借"),(100000003,"保守・サポート"),(100000004,"その他"),
    ]),
    (f"{PREFIX}_contract", f"{PREFIX}_auto_renewal", [
        (100000000,"あり"),(100000001,"なし"),
    ]),
    (f"{PREFIX}_contract", f"{PREFIX}_status", [
        (100000000,"有効"),(100000001,"期限切れ"),(100000002,"更新中"),(100000003,"解約"),(100000004,"下書き"),
    ]),
    (f"{PREFIX}_counterparty", f"{PREFIX}_company_type", [
...
def main():
    print("=" * 60)
    print("  Dataverse Setup: geek-contract 契約台帳管理ポータル")
    print("=" * 60)
