...
TABLES = [
    (f"{PREFIX}_contract",     "Contract",    "Contracts",    "契約台帳", "契約台帳一覧"),
    (f"{PREFIX}_counterparty", "Counterparty","Counterparties","取引先",  "取引先一覧"),
]
...
def set_display_names(to_english=True):
    for logical, en_d, en_p, jp_d, jp_p in TABLES:
        ...
        api_request(f"EntityDefinitions({mid})", {
            "@odata.type": "#Microsoft.Dynamics.CRM.EntityMetadata", "MetadataId": mid,
            "DisplayName": lbl(en_d if to_english else jp_d),
            "DisplayCollectionName": lbl(en_p if to_english else jp_p),
        }, method="PUT")
        print(f"  {logical} → {en_d if to_english else jp_d}")
if __name__ == "__main__":
    mode = sys.argv[1] if len(sys.argv) > 1 else "en"
    if mode == "en":
        print("=== テーブル表示名を英語に変更 ==="); set_display_names(True)
    elif mode == "jp":
        print("=== テーブル表示名を日本語に復元 ==="); set_display_names(False)
    else:
