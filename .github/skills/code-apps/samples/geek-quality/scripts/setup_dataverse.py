"""
Dataverse テーブル構築: {PREFIX}_inspection / {PREFIX}_defect
=========================================
品質検査ポータル（製造業・生産ライン検査）アプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_inspection", "display": "Inspection", "plural": "Inspections", "description": "検査記録",
        "columns": [
            {"logical": f"{PREFIX}_line",            "type": "String",   "display": "Line",            "maxLength": 200},
            {"logical": f"{PREFIX}_product",         "type": "String",   "display": "Product",         "maxLength": 200},
            {"logical": f"{PREFIX}_lot",             "type": "String",   "display": "Lot",             "maxLength": 100},
            {"logical": f"{PREFIX}_inspector",       "type": "String",   "display": "Inspector",       "maxLength": 200},
            {"logical": f"{PREFIX}_status",          "type": "Picklist", "display": "Status",
             "options": [(100000000,"Planned"),(100000001,"In Progress"),(100000002,"Completed")]},
            {"logical": f"{PREFIX}_inspection_date", "type": "DateTime", "display": "Inspection Date", "format": "DateOnly"},
            {"logical": f"{PREFIX}_inspected_qty",   "type": "Integer",  "display": "Inspected Qty",   "minValue": 0, "maxValue": 100000000},
            {"logical": f"{PREFIX}_defect_qty",      "type": "Integer",  "display": "Defect Qty",      "minValue": 0, "maxValue": 100000000},
            {"logical": f"{PREFIX}_notes",           "type": "Memo",     "display": "Notes",           "maxLength": 4000},
        ],
    },
    {
        "logical": f"{PREFIX}_defect", "display": "Defect", "plural": "Defects", "description": "不良記録",
        "columns": [
            {"logical": f"{PREFIX}_inspection_ref", "type": "String",   "display": "Inspection Ref", "maxLength": 100},
            {"logical": f"{PREFIX}_category",       "type": "String",   "display": "Category",       "maxLength": 200},
            {"logical": f"{PREFIX}_qty",            "type": "Integer",  "display": "Qty",            "minValue": 0, "maxValue": 100000000},
            {"logical": f"{PREFIX}_disposition",    "type": "Picklist", "display": "Disposition",
             "options": [(100000000,"Rework"),(100000001,"Scrap"),(100000002,"Use As-Is")]},
            {"logical": f"{PREFIX}_cause",          "type": "Memo",     "display": "Cause",          "maxLength": 2000},
        ],
    },
]
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_inspection", "検査記録", "検査記録一覧"),
    (f"{PREFIX}_defect",     "不良記録", "不良記録一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_inspection", f"{PREFIX}_name",            "件名"),
    (f"{PREFIX}_inspection", f"{PREFIX}_line",            "ライン"),
    (f"{PREFIX}_inspection", f"{PREFIX}_product",         "品目"),
    (f"{PREFIX}_inspection", f"{PREFIX}_lot",             "ロット番号"),
    (f"{PREFIX}_inspection", f"{PREFIX}_inspector",       "検査員"),
    (f"{PREFIX}_inspection", f"{PREFIX}_status",          "ステータス"),
    (f"{PREFIX}_inspection", f"{PREFIX}_inspection_date", "実施日"),
    (f"{PREFIX}_inspection", f"{PREFIX}_inspected_qty",   "検査数"),
    (f"{PREFIX}_inspection", f"{PREFIX}_defect_qty",      "不良数"),
    (f"{PREFIX}_inspection", f"{PREFIX}_notes",           "所見"),
    (f"{PREFIX}_defect", f"{PREFIX}_name",           "不良内容"),
    (f"{PREFIX}_defect", f"{PREFIX}_inspection_ref", "検査ID"),
    (f"{PREFIX}_defect", f"{PREFIX}_category",       "分類"),
    (f"{PREFIX}_defect", f"{PREFIX}_qty",            "数量"),
    (f"{PREFIX}_defect", f"{PREFIX}_disposition",    "処置"),
    (f"{PREFIX}_defect", f"{PREFIX}_cause",          "原因"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_inspection", f"{PREFIX}_status",      [(100000000,"計画"),(100000001,"実施中"),(100000002,"完了")]),
    (f"{PREFIX}_defect",     f"{PREFIX}_disposition", [(100000000,"手直し"),(100000001,"廃棄"),(100000002,"特別採用")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-quality 品質検査ポータル"); print("="*60)
