"""
Dataverse テーブル構築: {PREFIX}_store / {PREFIX}_store_audit / {PREFIX}_audit_item
=========================================
店舗運営ポータル（臨店チェック）アプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_store", "display": "Store", "plural": "Stores", "description": "店舗マスタ",
        "columns": [
            {"logical": f"{PREFIX}_region",  "type": "String",   "display": "Region",  "maxLength": 200},
            {"logical": f"{PREFIX}_manager", "type": "String",   "display": "Manager", "maxLength": 200},
            {"logical": f"{PREFIX}_status",  "type": "Picklist", "display": "Status",
             "options": [(100000000,"Open"),(100000001,"Renovation"),(100000002,"Closed")]},
        ],
    },
    {
        "logical": f"{PREFIX}_store_audit", "display": "Store Audit", "plural": "Store Audits", "description": "臨店チェック",
        "columns": [
            {"logical": f"{PREFIX}_store_ref",  "type": "String",   "display": "Store Ref",  "maxLength": 100},
            {"logical": f"{PREFIX}_auditor",    "type": "String",   "display": "Auditor",    "maxLength": 200},
            {"logical": f"{PREFIX}_status",     "type": "Picklist", "display": "Status",
             "options": [(100000000,"Planned"),(100000001,"In Progress"),(100000002,"Completed")]},
            {"logical": f"{PREFIX}_audit_date", "type": "DateTime", "display": "Audit Date", "format": "DateOnly"},
    ...
    {
        "logical": f"{PREFIX}_audit_item", "display": "Audit Item", "plural": "Audit Items", "description": "チェック項目",
        "columns": [
            {"logical": f"{PREFIX}_audit_ref", "type": "String",   "display": "Audit Ref", "maxLength": 100},
            ...
            {"logical": f"{PREFIX}_result",    "type": "Picklist", "display": "Result",
             "options": [(100000000,"Unchecked"),(100000001,"Pass"),(100000002,"Fail"),(100000003,"N/A")]},
...
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_store",       "店舗",          "店舗一覧"),
    (f"{PREFIX}_store_audit", "臨店チェック",  "臨店チェック一覧"),
    (f"{PREFIX}_audit_item",  "チェック項目",  "チェック項目一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_store", f"{PREFIX}_name",    "店舗名"),
    (f"{PREFIX}_store", f"{PREFIX}_region",  "地域"),
    (f"{PREFIX}_store", f"{PREFIX}_manager", "店長"),
    (f"{PREFIX}_store", f"{PREFIX}_status",  "ステータス"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_name",       "件名"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_store_ref",  "店舗ID"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_auditor",    "巡回担当"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_status",     "ステータス"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_audit_date", "実施日"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_score",      "スコア"),
    (f"{PREFIX}_store_audit", f"{PREFIX}_notes",      "所感"),
    (f"{PREFIX}_audit_item", f"{PREFIX}_name",      "項目名"),
    (f"{PREFIX}_audit_item", f"{PREFIX}_audit_ref", "臨店ID"),
    (f"{PREFIX}_audit_item", f"{PREFIX}_category",  "カテゴリ"),
    (f"{PREFIX}_audit_item", f"{PREFIX}_result",    "判定"),
    (f"{PREFIX}_audit_item", f"{PREFIX}_comment",   "コメント"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_store",       f"{PREFIX}_status", [(100000000,"営業中"),(100000001,"改装中"),(100000002,"閉店")]),
    (f"{PREFIX}_store_audit", f"{PREFIX}_status", [(100000000,"予定"),(100000001,"実施中"),(100000002,"完了")]),
    (f"{PREFIX}_audit_item",  f"{PREFIX}_result", [(100000000,"未確認"),(100000001,"合格"),(100000002,"不合格"),(100000003,"対象外")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-store 店舗運営ポータル"); print("="*60)
