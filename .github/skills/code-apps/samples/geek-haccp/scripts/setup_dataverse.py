"""
Dataverse テーブル構築: {PREFIX}_checkpoint / {PREFIX}_measurement
=========================================
衛生管理ポータル（飲食・HACCP 温度/衛生点検）アプリ用テーブル。
点検項目マスタ（管理基準の上下限）と測定記録（値・時間帯・是正措置）の 2 テーブル。
温度は氷点下（冷凍 -18℃ 等）を扱うため Decimal 列の MinValue は負値を許可する。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_checkpoint", "display": "Checkpoint", "plural": "Checkpoints", "description": "点検項目マスタ",
        "columns": [
            {"logical": f"{PREFIX}_category",  "type": "Picklist", "display": "Category",
             "options": [(100000000,"Refrigeration"),(100000001,"Heating"),(100000002,"Storage/Display"),(100000003,"Hygiene")]},
            {"logical": f"{PREFIX}_unit",      "type": "String",  "display": "Unit",      "maxLength": 20},
            {"logical": f"{PREFIX}_min_value", "type": "Decimal", "display": "Min Value", "precision": 1, "minValue": -1000, "maxValue": 100000},
            {"logical": f"{PREFIX}_max_value", "type": "Decimal", "display": "Max Value", "precision": 1, "minValue": -1000, "maxValue": 100000},
        ],
    },
    {
        "logical": f"{PREFIX}_measurement", "display": "Measurement", "plural": "Measurements", "description": "測定記録",
        "columns": [
            {"logical": f"{PREFIX}_checkpoint_ref", "type": "String",   "display": "Checkpoint Ref", "maxLength": 100},
            {"logical": f"{PREFIX}_value",          "type": "Decimal",  "display": "Value",          "precision": 1, "minValue": -1000, "maxValue": 100000},
            {"logical": f"{PREFIX}_measured_date",  "type": "DateTime", "display": "Measured Date",  "format": "DateOnly"},
            {"logical": f"{PREFIX}_time_slot",      "type": "Picklist", "display": "Time Slot",
             "options": [(100000000,"Before Open"),(100000001,"Morning"),(100000002,"Afternoon"),(100000003,"After Close")]},
...
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_checkpoint",  "点検項目", "点検項目一覧"),
    (f"{PREFIX}_measurement", "測定記録", "測定記録一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_checkpoint", f"{PREFIX}_name",      "項目名"),
    (f"{PREFIX}_checkpoint", f"{PREFIX}_category",  "カテゴリ"),
    (f"{PREFIX}_checkpoint", f"{PREFIX}_unit",      "単位"),
    (f"{PREFIX}_checkpoint", f"{PREFIX}_min_value", "下限値"),
    (f"{PREFIX}_checkpoint", f"{PREFIX}_max_value", "上限値"),
    (f"{PREFIX}_measurement", f"{PREFIX}_name",           "記録名"),
    (f"{PREFIX}_measurement", f"{PREFIX}_checkpoint_ref", "点検項目ID"),
    (f"{PREFIX}_measurement", f"{PREFIX}_value",          "測定値"),
    (f"{PREFIX}_measurement", f"{PREFIX}_measured_date",  "測定日"),
    (f"{PREFIX}_measurement", f"{PREFIX}_time_slot",      "時間帯"),
    (f"{PREFIX}_measurement", f"{PREFIX}_inspector",      "点検者"),
    (f"{PREFIX}_measurement", f"{PREFIX}_action",         "是正措置"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_checkpoint",  f"{PREFIX}_category",  [(100000000,"冷蔵・冷凍"),(100000001,"加熱"),(100000002,"保管・陳列"),(100000003,"衛生")]),
    (f"{PREFIX}_measurement", f"{PREFIX}_time_slot", [(100000000,"開店前"),(100000001,"午前"),(100000002,"午後"),(100000003,"閉店後")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-haccp 衛生管理ポータル"); print("="*60)
