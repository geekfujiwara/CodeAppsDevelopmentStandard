"""
Dataverse テーブル構築: {PREFIX}_approval_request / {PREFIX}_approval_step
=========================================
稟議申請ポータルアプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_approval_request", "display": "Approval Request", "plural": "Approval Requests", "description": "稟議申請",
        "columns": [
            {"logical": f"{PREFIX}_category",     "type": "String",   "display": "Category",     "maxLength": 200},
            {"logical": f"{PREFIX}_applicant",    "type": "String",   "display": "Applicant",    "maxLength": 200},
            ...
            {"logical": f"{PREFIX}_stage",        "type": "Picklist", "display": "Stage",
             "options": [(100000000,"Draft"),(100000001,"Manager Review"),(100000002,"Director Review"),(100000003,"Approved"),(100000004,"Rejected")]},
            ...
            {"logical": f"{PREFIX}_request_date", "type": "DateTime", "display": "Request Date", "format": "DateOnly"},
    ...
    {
        "logical": f"{PREFIX}_approval_step", "display": "Approval Step", "plural": "Approval Steps", "description": "承認履歴",
        "columns": [
            {"logical": f"{PREFIX}_request_ref",  "type": "String",   "display": "Request Ref",  "maxLength": 100},
            {"logical": f"{PREFIX}_approver",     "type": "String",   "display": "Approver",     "maxLength": 200},
            {"logical": f"{PREFIX}_decision",     "type": "Picklist", "display": "Decision",
             "options": [(100000000,"Approved"),(100000001,"Returned"),(100000002,"Rejected")]},
            {"logical": f"{PREFIX}_comment",      "type": "Memo",     "display": "Comment",      "maxLength": 2000},
            {"logical": f"{PREFIX}_decided_date", "type": "DateTime", "display": "Decided Date", "format": "DateOnly"},
        ],
    },
]
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_approval_request", "稟議申請", "稟議申請一覧"),
    (f"{PREFIX}_approval_step",    "承認履歴", "承認履歴一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_approval_request", f"{PREFIX}_name",         "件名"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_category",     "種別"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_applicant",    "申請者"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_department",   "部門"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_amount",       "金額"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_stage",        "承認ステージ"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_priority",     "優先度"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_request_date", "申請日"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_description",  "申請内容"),
    (f"{PREFIX}_approval_request", f"{PREFIX}_notes",        "備考"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_name",         "ステップ名"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_request_ref",  "申請ID"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_approver",     "承認者"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_decision",     "判定"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_comment",      "コメント"),
    (f"{PREFIX}_approval_step", f"{PREFIX}_decided_date", "判定日"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_approval_request", f"{PREFIX}_stage",    [(100000000,"下書き"),(100000001,"課長承認待ち"),(100000002,"部長承認待ち"),(100000003,"承認済み"),(100000004,"却下")]),
    (f"{PREFIX}_approval_request", f"{PREFIX}_priority", [(100000000,"低"),(100000001,"中"),(100000002,"高"),(100000003,"緊急")]),
    (f"{PREFIX}_approval_step",    f"{PREFIX}_decision", [(100000000,"承認"),(100000001,"差戻し"),(100000002,"却下")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-approval 稟議申請ポータル"); print("="*60)
