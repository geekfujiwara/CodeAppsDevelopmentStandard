"""
Dataverse テーブル構築: {PREFIX}_suggestion
=========================================
改善提案ポータルアプリ用テーブル。
"""
...
TABLES = [
    {
        "logical": f"{PREFIX}_suggestion", "display": "Suggestion", "plural": "Suggestions", "description": "改善提案",
        "columns": [
            {"logical": f"{PREFIX}_category",      "type": "String",   "display": "Category",      "maxLength": 200},
            {"logical": f"{PREFIX}_proposer",      "type": "String",   "display": "Proposer",      "maxLength": 200},
            ...
            {"logical": f"{PREFIX}_status",        "type": "Picklist", "display": "Status",
             "options": [(100000000,"Submitted"),(100000001,"Under Review"),(100000002,"Adopted"),(100000003,"Implemented"),(100000004,"Declined")]},
            {"logical": f"{PREFIX}_votes",         "type": "Integer",  "display": "Votes",         "minValue": 0, "maxValue": 1000000},
            {"logical": f"{PREFIX}_proposed_date", "type": "DateTime", "display": "Proposed Date", "format": "DateOnly"},
            {"logical": f"{PREFIX}_description",   "type": "Memo",     "display": "Description",   "maxLength": 4000},
            {"logical": f"{PREFIX}_effect",        "type": "Memo",     "display": "Effect",        "maxLength": 4000},
        ],
    },
]
LOOKUPS = []
LOCALIZE_TABLES = [
    (f"{PREFIX}_suggestion", "改善提案", "改善提案一覧"),
]
LOCALIZE_COLUMNS = [
    (f"{PREFIX}_suggestion", f"{PREFIX}_name",          "タイトル"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_category",      "分野"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_proposer",      "提案者"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_department",    "部門"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_status",        "ステータス"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_votes",         "いいね数"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_proposed_date", "提案日"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_description",   "提案内容"),
    (f"{PREFIX}_suggestion", f"{PREFIX}_effect",        "期待効果"),
]
LOCALIZE_OPTIONS = [
    (f"{PREFIX}_suggestion", f"{PREFIX}_status", [(100000000,"提案"),(100000001,"検討中"),(100000002,"採用"),(100000003,"実施済み"),(100000004,"見送り")]),
]
...
def main():
    print("="*60); print("  Dataverse Setup: geek-kaizen 改善提案ポータル"); print("="*60)
