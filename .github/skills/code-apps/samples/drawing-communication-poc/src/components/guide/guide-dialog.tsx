      ...
      <DialogContent className="overflow-hidden p-0 sm:max-w-2xl">
        ...
        <div className="flex flex-col gap-4 border-t px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2" aria-label="スライド位置">
            {GUIDE_SLIDES.map((item, position) => (
              <button
                key={item.id}
                type="button"
                aria-label={`${position + 1} 枚目: ${item.title}`}
                aria-current={position === index}
                onClick={() => setIndex(position)}
                className={cn(
                  "h-2 rounded-full transition-all duration-200",
                  position === index ? "w-6 bg-primary" : "w-2 bg-muted-foreground/30 hover:bg-muted-foreground/60",
                )}
              />
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button variant="ghost" size="sm" disabled={index === 0} onClick={() => setIndex((value) => value - 1)}>
