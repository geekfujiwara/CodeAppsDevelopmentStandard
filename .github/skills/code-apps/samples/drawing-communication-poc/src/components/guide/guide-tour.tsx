...
function cardStyle(rect: Rect | null): CSSProperties {
  ...
  if (!rect) {
    return { width, top: Math.round(window.innerHeight / 2 - 120), left: Math.round((window.innerWidth - width) / 2) }
  }
...
export function GuideTour({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (cancelled) return
      const element = findTarget(step.autoClick)
      if (element) {
        element.click()
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const next = { top: bounds.top, left: bounds.left, width: bounds.width, height: bounds.height }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[500]" role="dialog" aria-modal="true" aria-label="使い方ガイド">
      ...
      <div className="absolute rounded-xl border bg-background p-5 shadow-2xl transition-all duration-300" style={cardStyle(rect)}>
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" disabled={index === 0} onClick={() => setIndex((value) => value - 1)}>
              <ArrowLeft className="mr-1 h-4 w-4" />
              戻る
            </Button>
            <Button size="sm" onClick={() => (isLast ? finish() : setIndex((value) => value + 1))}>
              {isLast ? "完了" : "次へ"}
