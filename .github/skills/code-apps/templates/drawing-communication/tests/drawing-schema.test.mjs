...
test("agent-skill の sample.json は TypeScript 側でも同じハッシュになる（Python との相互運用）", () => {
  // sample.json を作り直したら tests/test_renderer.py の SAMPLE_HASH も取り直すこと
  const sample = parseDrawing(readFileSync(new URL("../agent-skill/sample.json", import.meta.url), "utf-8"))
  assert.equal(hashDrawing(sample), "f278d25fe021b1df")
  assert.equal(sample.annotations.length, 2)
  assert.ok(buildScene(sample).primitives.length > 30)
})
