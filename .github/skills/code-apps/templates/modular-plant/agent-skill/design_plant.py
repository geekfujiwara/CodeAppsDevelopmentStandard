...
def terminal(nodes, reference):
    node = next((node for node in nodes if node["id"] == reference["nodeId"]), None)
    if node is None or reference["portId"] not in ports(node):
        raise ValueError(f"Unknown terminal: {reference['nodeId']}:{reference['portId']}")
...
def module_for(design, unit):
    module = next((module for module in design["modules"] if module["id"] == unit["moduleId"]), None)
    if module is None:
        raise ValueError(f"Unknown module: {unit['moduleId']}")
    return module
...
def check_schema(design):
    encoded = json.dumps(design, ensure_ascii=False, allow_nan=False).encode("utf-8")
...
def create_layout(source, requirements=None, step=2):
    ...
    if not math.isfinite(step) or step < 1 or step > 20:
