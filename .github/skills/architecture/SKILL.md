...
一言リクエストだけで適切なアーキテクチャが判断しにくいときは、ユーザーに以下を依頼する:

> 現在お使いの Excel ファイル・業務フロー図・画面イメージ・既存の帳票などがあれば共有してください。
> `/spec-to-markdown` でドキュメントから要件を整理することもできます。

### 要件が明確になったらアーキテクチャ設計へ

以下の判断フローチャートに従ってコンポーネントを選定し、設計提案に進む。
...
> **理由**: Code Apps は保守性・拡張性・UI カスタマイズ性に優れ、長期的に最もメリットが大きい。
> Model-Driven Apps はテーブル定義から UI が自動生成される利点があるが、カスタマイズ性の制約により
> 後から Code Apps に移行するコストが発生しやすい。新規開発では最初から Code Apps を選択する。
