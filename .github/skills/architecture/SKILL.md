...
5. そのうえで下の**ライト実装 / 本格実装**を選ぶ
6. 選択結果をまとめて [`agent365` スキル](../agent365/SKILL.md) の Step 0 へ引き渡す
...
| 実施 Step | `agent365` スキルの Step 6・14 を省略 | 全 Step |

> **迷ったらライト実装から始める**。エージェント定義・Teams パッケージ・スクリプトはそのまま流用でき、
> Step 6（Agent 365 ブループリント）と Step 14（インスタンス SP への同意）を追加し、
