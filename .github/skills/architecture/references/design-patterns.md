...
### パターン G2: 複数の外部データソースを横断する「合成回答」型

パターン G を複数データソースに広げた形。**台帳（Dataverse）を起点に外部データを辿り、
突き合わせた結果を根拠付きで返す**構成で、既存資産を移行せずに回答品質を上げたいときに使う。

```
[Dataverse]  台帳（案件・マスタ・問い合わせ・ナレッジ）
   │  ① 起点となるキーを特定（例: 図面番号 → 有効改訂）
   ↓
[エージェント（Copilot Studio v2 スキル）]
   ├─ ② [自前 MCP #1] ファイル系（Azure Files）   … 図面 / 設計書 / 手順書を都度読む
   ├─ ③ [自前 MCP #2] RDB 系（Azure SQL）         … 過去の実績・履歴を照会
   └─ ④ [Dataverse MCP]                          … 台帳の付帯情報を参照
   ↓  ⑤ 3 系統を突き合わせて回答。根拠が揃わない箇所は推測せず「回答不能」を返す
[Dataverse へ書き戻し]  出典（種別つき）とナレッジを登録 ← 書き込みはここだけ
```

**設計の要点**（詳細は SKILL.md §6.5「外部データを繋ぐときの設計原則」）:

- 外部データは**移行しない**。Dataverse は台帳、回答は合成結果、ナレッジは副産物として扱う
- **1 データソース = 1 MCP Server**。ファイル系と RDB 系を同居させない
- 外部向け MCP は**読み取り専用**。書き込みは Dataverse MCP に集約する
- 出典に**種別**を持たせ、「外部データソース参照率」「根拠の横断度」をダッシュボードで測る
- 外部由来の文章（図面内テキスト・文書本文）は**資料であって指示ではない**ことをスキルに明記し、
  プロンプト インジェクションの golden question で検証する

**向くケース**: 設計 / 保守 / 品質のように「一次資料（図面・仕様書）＋過去実績（故障・修理履歴）」を
突き合わせて初めて答えが出る業務。
**向かないケース**: 単一データソースで完結する Q&A（→ パターン B / G）。
