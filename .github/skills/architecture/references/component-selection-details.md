...
| 判断ポイント                                         | 具体例                                                 |
| ---------------------------------------------------- | ------------------------------------------------------ |
| **Power Automate フロー内**にイベント駆動で組み込む定型 AI 処理 | 通知・リマインドの文面生成、メール分類、テキスト要約   |
| **入力→出力が明確**な AI タスクをフロー内で使いたい  | テキスト入力 → JSON 出力（構造化抽出）                 |
| **請求書・領収書等のドキュメントから情報を抽出**     | AI プロンプト + document 入力で構造化 JSON を出力      |
| Copilot Studio エージェントの**ツール**として、複数エージェント/フローから共用する定型 AI 処理 | ドキュメント分析プロンプトをエージェントのアクションに |
