# 外部公開 Web サイト埋め込みパターン（iframe・レガシー / 非推奨）

> ⚠️ **これはレガシーパターン。外部公開の標準は WebChat UI（[webchat-sdk-embed.md](webchat-sdk-embed.md)）。**
> iframe 方式は UI カスタマイズ・プログラム的メッセージ送信ができないため、
> PoC・動作確認以外では使わない。新規構築では WebChat SDK ＋ 既存デザインテンプレートを使うこと。

外部顧客向けにエージェントを Web サイトに公開する際の簡易パターン。
