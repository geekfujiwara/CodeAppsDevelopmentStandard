...
## 1. 検索用テキストと表示用ページ画像は投入時に生成する

PDF / CAD / Office をサーバー内で解析すると、重い依存・フォント・OCR・タイムアウトを Functions に持ち込むことになる。
**検索用の抽出は投入パイプライン側の責務**とし、MCP は抽出済みテキストを読む。
Code App に表示する PDF ページ画像も投入時に描画し、図番・改訂・ページ・SHA-256を含む検証済み
JSONとして Dataverse の索引へ保存する。Code App は Dataverse から読み、MCP Serverへ直接接続しない。
既存資料の移行時だけ、同じ検証処理を使うバックフィルを1回実行する。

オンデマンド描画 API を診断・バックフィル用に残す場合も、入力は `sourceId` のみに限定し、PDF未配置は
`404` と機械判定可能な状態コードを返す。描画器障害の `502` と混同しない。
[認可・上限・フォント検証](indexed-file-db-access.md) を適用する。
