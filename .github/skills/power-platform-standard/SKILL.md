...
| **C# プラグインは署名必須**                            | `pac plugin init`（`--skip-signing` なし）で署名付き `.snk` を生成。署名なしは `0x8004416c: Public assembly must have public key token` エラー |
| **プラグイン初回登録は API 経由**                      | `pac plugin push` は既存 `--pluginId` が必要。初回は `pluginassemblies` → `plugintypes` → `sdkmessageprocessingsteps` の順に API POST で登録 |
| **プラグイン更新は pac plugin push**                   | 初回登録後は `pac plugin push --pluginId {assembly-id}` で DLL を更新可能                                              |
| **AI Builder モデル有効化は UI のみ**                  | `msdyn_aimodel` の `statecode`/`statuscode`/`activerunconfigurationid` は API で設定不可。make.powerapps.com → AI Hub → テスト → 公開 で有効化 |
| **AI Builder FK は @odata.bind パターン**              | `_msdyn_templateid_value` ではなく `msdyn_TemplateId@odata.bind: /msdyn_aitemplates(GUID)` を使用。`msdyn_AIModelId`, `msdyn_TrainedModelAIConfigurationPareId` も同様 |
| **AI Builder Training Config に msdyn_modelrundataspecification 不可** | 含めると `Unexpected parameter` エラー。Training Config からは除外すること |
