...
3. **以下のどのパターンに近いですか？**
   ...
   E) **360度ビュー** — マスターデータを選択し、関連トランザクションをタブ切替で全方位確認（→ `360-view-pattern` スキル参照）
   F) **日本地図ダッシュボード** — 都道府県別データの地図可視化・地域分析
...
### Pattern E: 360度ビュー（マスター詳細 + 関連トランザクション）

**向いている場面**: 設備台帳、顧客管理、IT資産管理、製品管理 — マスターレコードと関連データの全方位確認

> **詳細な実装パターンは `360-view-pattern` スキル（`.github/skills/360-view-pattern/SKILL.md`）を参照。**
> ここでは Generative Pages 固有の構成要素のみ記載。

**構成要素（Tier順）**:

| Tier | コンポーネント | 説明 |
|------|--------------|------|
| 1 | マスター一覧（Card リスト） | 検索 + カードリストでマスター選択 |
| 1 | プロファイルセクション | KPI カード4列 + 基本属性グリッド |
| 1 | タブナビゲーション | `TabList` で概要/履歴A/履歴B/分析/タイムラインを切替 |
| 1 | トランザクション DataGrid | 検索 + DataGrid + モーダル編集 |
| 1 | モーダル編集フォーム | Dialog + ボタン式 Choice + トースト通知 |
| 2 | D3 チャート（棒・ドーナツ） | 月別推移 + ステータス分布 |
| 2 | タイムライン | 全トランザクションを時系列マージ表示 |
| 3 | 概要タブ | 詳細属性 + 直近アクティビティ |
| 3 | トレンドライン | D3 area + line で推移表示 |
...
[← 戻る] マスター名
[KPI] [KPI] [KPI] [KPI]           ← 4列グリッド
[タイプ | 場所 | 担当者 | 登録日]   ← 属性カード

[概要] [履歴A (12)] [履歴B (5)] [分析] [タイムライン]  ← タブ
─────────────────────────────────────────────
（選択中タブのコンテンツ）
```

### Pattern F: 日本地図ダッシュボード
