...
（[SKILL.md](../SKILL.md) の Step 0〜11）をどう拡張して複数エージェントに適用するか**の標準フローを示す。
...
Agent 365 のエージェント ID ブループリント（Step 6、本格実装のインスタンス化用）は
...
このスキルのデフォルト実装（上記のテンプレート・Step 0〜11）は **(A)** を前提にしている。
