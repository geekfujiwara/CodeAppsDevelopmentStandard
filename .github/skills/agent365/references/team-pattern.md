...
（[SKILL.md](../SKILL.md) の Step 0〜12）をどう拡張して複数エージェントに適用するか**の標準フローを示す。
...
Agent 365 のエージェント ID ブループリント（Step 7、本格実装のインスタンス化用）は
...
このスキルのデフォルト実装（上記のテンプレート・Step 0〜12）は **(A)** を前提にしている。
...
**Step 8（Azure Bot Service）・Step 10（Graph API での Teams 組織カタログ公開）は実施しない**。
以下がそろっていれば「テンプレートとしての公開」は完了している。

> **SKILL.md Step 6** はまさにこの判断を AskUserQuestion で明示的に確認するゲートである。
...
**利用者が実際に Teams で使い始めたいタイミングで改めて Step 8〜10 を実施すればよい**
...
- **「エージェントテンプレートとしての公開」と「Teams への実配信」は別のマイルストーン**。
  ...
  コミット）だけでも「テンプレートとして公開した」と言える状態になる。Step 8（Azure Bot
  Service、課金あり）・Step 10（Graph API での組織カタログ公開、テナント全員に見える状態に
