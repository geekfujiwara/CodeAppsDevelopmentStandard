// エージェントに「自分でコードを書いて動かす作業環境」を持たせるローカル ツール群（B12）。
//
// ここで価値を生むのは個々のツールではなく **ループ**（書く → 動かす → 出力を読む → 直す）。
// 失敗したらエラー本文を読んで直し、もう一度呼ぶ。実行結果は stdout / stderr / 最後の式の値を
// そのままモデルへ返す。整形して隠さない。
//
// アプリ設定（__ が階層区切り）:
//   Sandbox__Enabled = true（Endpoint / ApiVersion / TimeoutSeconds は CodeSandbox 側）
//
// デザイン キット（任意）:
    ...
    ILogger<SandboxTools> logger)
{
                ...
                Schema("""
                    {
                      ...
                      "properties": {
                        ...
                        "owner": { "type": "string", "description": "依頼元のメール アドレス。分かるなら必ず入れる。" },
                        "sensitivity": {
                          "type": "string",
                          "enum": ["public", "internal", "personal"],
                          "description": "中身の取り扱い区分。迷ったら厳しい方を選ぶ。"
                        }
                      },
                      "required": ["path"]
                    }
        ...
        string? accessToken, string source, CancellationToken cancellationToken)
    {
        ...
        const string GraphRoot = "https://graph.microsoft.com/v1.0";
        string route = isUrl
