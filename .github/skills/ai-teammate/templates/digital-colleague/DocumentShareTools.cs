// 保存済みのファイルを、依頼元以外の人に渡すためのローカル ツール群（B14）。
//
// このファイルの要点は **判断をモデルに任せない** こと。共有リンクは取り消せないので、
// 可否は台帳の内容と、そのターンの話し相手から C# で決める。
//
// 判定:
    ...
    ILogger<DocumentShareTools> logger)
{
    ...
    private async Task<string> ListAsync(string accessToken, JsonElement arguments, CancellationToken cancellationToken)
    {
        ...
        foreach ((string name, long size, string? modified) in files.Take(40))
        {
            ...
            lines.Add(
                ...
                + $"  区分: {Sensitivity.Label(record?.SensitivityLevel)}"
    ...
    private Task<string> ClassifyAsync(JsonElement arguments, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult($"「{name}」を{Sensitivity.Label(sensitivity)}として記録しました。");
    }
        ...
        string accessToken, string? userEmail, JsonElement arguments, CancellationToken cancellationToken)
    {
        ...
        DocumentRecord? record = ledger.Find(name);
        if (record?.SensitivityLevel is null)
        {
        ...
        string sensitivity = record.SensitivityLevel;
        ...
        bool needsConsent = sensitivity == Sensitivity.Personal
            || (sensitivity == Sensitivity.Internal && !recipientIsInternal);
        ...
        return $"「{record.Name}」（{Sensitivity.Label(sensitivity)}）の共有リンクを発行しました。\n"
        ...
        DocumentRecord record, string recipient, string? userEmail, JsonElement arguments)
    {
        ...
        {
            return Task.FromResult(
                $"共有できません。「{record.Name}」は{Sensitivity.Label(record.SensitivityLevel)}ですが、"
        ...
        return Task.FromResult(
            $"まだ共有できません。「{record.Name}」は{Sensitivity.Label(record.SensitivityLevel)}のため、"
            ...
            + $"許可が下りるのは {record.OwnerEmail} さん本人が Teams で直接返事をしたときだけ。ここで先に渡してはいけない。");
    }
    ...
    private Task<string> DecideAsync(string? userEmail, JsonElement arguments, CancellationToken cancellationToken)
    {
        ...
        {
            return Task.FromResult(
                "この経路では共有の可否を記録できません。許可はご本人が Teams で直接伝える必要があります。"
                + "そのようにご案内すること。");
        }
