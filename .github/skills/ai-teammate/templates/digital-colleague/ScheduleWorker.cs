...
// MailboxWorker と同じ型（BackgroundService ＋ AgenticTokenSource ＋ AgentBrain）。
//
// アプリ設定（__ が階層区切り）:
    ...
    ILogger<ScheduleWorker> logger) : BackgroundService
{
    ...
    private async Task RunAsync(ScheduledJob job, McpToolset toolset, CancellationToken cancellationToken)
    {
        ...
        string summary = await brain.CompleteAsync(
            history, BuildContext(job, toolset), toolset, cancellationToken,
    ...
    private static string BuildContext(ScheduledJob job, McpToolset toolset)
    {
        ...
        if (job.Channel == "mail")
        {
            delivery.AppendLine($"   reply_mail は使えないので、mail ツールが構成されていなければメール配信できないと判断すること。");
            delivery.AppendLine($"   件名は「{job.Subject ?? job.Title}」、宛先は `{job.Recipient}`。");
        }
        else
        {
