// Web 検索（B10）。Azure OpenAI Responses API 組み込みの web_search（Grounding with Bing）を使う。
// 追加の Azure リソースは不要。App Service の UAMI（Cognitive Services OpenAI User）だけで動く。
// アプリ設定: WebSearch__Enabled / WebSearch__Deployment（省略時は AzureOpenAI__Deployment）。
