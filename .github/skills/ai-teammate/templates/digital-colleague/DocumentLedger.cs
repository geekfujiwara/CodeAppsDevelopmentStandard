// エージェントが作って渡したファイルの台帳（B14）。**誰の依頼で作ったか**と
// **中身の取り扱い区分**を残す。共有リンクは一度渡すと取り消せないので、
// 可否はモデルの記憶ではなく記録から判断する。
//
...
public sealed class DocumentRecord
{
    ...
    {
        string level = Sensitivity.Label(SensitivityLevel);
        return $"- {Name}\n  区分: {level}"
...
public sealed class DocumentLedger
{
    ...
    public void Record(string name, string? itemId, string? webUrl, string? ownerEmail, string? sensitivity, string? summary)
    {
        lock (_gate)
        {
            ...
            existing.SensitivityLevel = Sensitivity.Normalize(sensitivity) ?? existing.SensitivityLevel;
