        ...
        .AddJwtBearer(options =>
        {
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                OnTokenValidated = context =>
                {
                    ...
                    // Note: the SDK extension ClaimsIdentity.IsTenantIdIssuerValid() is not available in the
                    // referenced package version, so the equivalent check is done inline here.
                    if (!isBotFrameworkToken && issuer != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(tid) && issuer.IndexOf(tid, StringComparison.OrdinalIgnoreCase) < 0)
                        {
