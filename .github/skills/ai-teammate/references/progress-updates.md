...
- **固有名詞・件数・いま見ているものを入れる。** 入っていない文は送らない
- 短い用件では呼ばない
...
**呼ばなくても仕事は完了する**ので、モデルは黙って作業を続けてしまう。
無言が続いているときだけ、**ツールの戻り値に催促を 1 行足す**（`AgentProgress.Nudge()`）。

```csharp
// フェンスの外側に付ける。中に入れると、取り込んだ文章の中の指示と区別できなくなる
return progress?.Nudge() is { } nudge ? payload + nudge : payload;
```

自動通知（§3）を**催促より後ろに置く**。先に定型文が出ると、
エージェントが自分の言葉で言う前に「もう伝えた」状態になる（→ [troubleshooting.md](troubleshooting.md) #69）。
...
| `Agent__Progress__NudgeSeconds` | `15` | ツール結果に催促を添え始めるまでの秒数 |
