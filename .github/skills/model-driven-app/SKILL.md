...
デプロイスクリプト・ナビゲーション設計パターンの詳細は [デプロイリファレンス](deploy-reference.md) を参照。
...
| **テーブルアイコンは SVG で設定**                   | `IconVectorName` + Web Resource (type=11)。詳細は `icon-creation` スキル参照 |
| **`IconVectorName` は PUT で設定**                  | `MSCRM.MergeLabels: true` 必須。詳細は `icon-creation` スキル参照            |
