...
export async function apiPost<T>(path: string, body: unknown): Promise<T> {
  ...
  if (token)
    (headers as Record<string, string>)["__RequestVerificationToken"] = token;
  const res = await fetch(`${BASE}/${path}`, {
    method: "POST",
    credentials: "same-origin",
    headers,
...
### 2. すべての Web API 呼び出しに `__RequestVerificationToken` が必須（テンプレート + JS 両面対策）

> MS Learn 公式: **"All Web API calls must include a Cross-Site Request Forgery (CSRF) token."**
> POST/PATCH/DELETE はもちろん、**GET でも付与する**のが安全（GET で 403 が出る場合の典型原因）。
> トークンは `/_layout/tokenhtml` から取得し、403 を受けたらキャッシュをクリアして再取得する。
...
{% endif %} {{ request_verification_token }} {# ← これが hidden input を出力する
#}
<div id="root"></div>
