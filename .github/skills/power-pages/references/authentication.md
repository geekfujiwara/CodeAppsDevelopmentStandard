...
export function useAuth() {
  ...
  // ── ① シングルサインオン（SSO ログイン）──
  // /SignIn から anti-forgery token を取得し、直接 Entra ID に POST。
  // クラシックのプロバイダー選択ページを表示せず一気に Entra ID へ遷移させる。
...
| 方式 | 挙動 | 用途 |
|------|------|------|
| **form POST → ExternalLogin**（上記 `login`） | プロバイダー選択ページをスキップし即 Entra ID へ | プロバイダーが 1 つのとき推奨 |
