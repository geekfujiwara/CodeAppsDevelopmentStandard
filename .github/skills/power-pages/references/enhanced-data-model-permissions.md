...
2. **テーブル権限への Web ロール紐付け** → `powerpagecomponent_powerpagecomponent` 自己参照 N:N（API）
...
> **`mspp_entitypermission_webrole` の N:N は API で永続化しない**。代わりに `powerpagecomponent_powerpagecomponent` 自己参照 N:N を使う（詳細は後述）。
