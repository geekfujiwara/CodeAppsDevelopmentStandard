...
Enhanced Data Model では、Power Pages ランタイムが **`mspp_` プレフィックスのテーブル**（mspp_entitypermission, mspp_webrole 等）を使用してセキュリティ評価を行う。`mspp_entitypermission_webrole` の N:N は API で永続化しないバグがあるが、**`powerpagecomponent_powerpagecomponent` 自己参照 N:N で完全自動化が可能**。
...
★ 正解:
powerpagecomponent (type=18) ←→ powerpagecomponent (type=11)
  via powerpagecomponent_powerpagecomponent (自己参照 N:N) ← ✅ API で永続化する
```
...
## ✅ 解決済み: powerpagecomponent_powerpagecomponent 自己参照 N:N

### 問題（mspp_entitypermission_webrole は永続化しない）

`mspp_entitypermission_webrole` の N:N リレーションシップは、Dataverse Web API で 204 を返すが**永続化しない**。
...
### ✅ 正解: `powerpagecomponent_powerpagecomponent` 自己参照 N:N

Enhanced Data Model では、テーブル権限(type=18)も Web ロール(type=11)も同じ `powerpagecomponent` テーブルに格納される。この2つを紐付けるには **自己参照 N:N リレーション `powerpagecomponent_powerpagecomponent`** を使う。

```python
"""Table Permission → Web Role リンク（検証済みパターン）"""
import requests
...
# テーブル権限の powerpagecomponentid
PERM_ID = "01a5cf4c-ee5a-f111-a825-7ced8d3ce773"
# Web Role (Authenticated Users) の powerpagecomponentid
ROLE_PPC_ID = "856d6581-b7fd-40d5-ba83-3cf76b3eb9b5"
...
# 検証
r = requests.get(
    f"{DATAVERSE_URL}/api/data/v9.2/powerpagecomponents({PERM_ID})/powerpagecomponent_powerpagecomponent"
    "?$select=powerpagecomponentid,name,powerpagecomponenttype",
    headers=headers
)
linked = r.json()["value"]
# [{"name": "Authenticated Users", "powerpagecomponenttype": 11, ...}]
```

### 発見の経緯

1. `RelationshipDefinitions/Microsoft.Dynamics.CRM.ManyToManyRelationshipMetadata` を照会
2. `powerpagecomponent` の N:N に `powerpagecomponent_powerpagecomponent`（自己参照）が存在
3. テーブル権限(type=18) → Web ロール(type=11) の方向で `$ref` POST → **204 & 永続化確認**
4. intersect テーブル `powerpagecomponent_powerpagecomponentset` にレコードが生成される

### Web Role の powerpagecomponentid 取得方法

```python
# Authenticated Users ロールを取得
r = requests.get(
...
| 3 | **Web Role 紐付け (N:N)** | **`powerpagecomponent_powerpagecomponent` 自己参照 N:N** | ✅ 可能 |
