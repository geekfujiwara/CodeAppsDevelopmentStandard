...
### 教訓 14: フォントはコード側（HTML + CSS + Tailwind theme）で一元管理する

> **実案件 (M365 Status Portal) で公開サイトにフォント未設定だった問題の対処で確定。**

Code Site (SPA) のフォントは Dataverse 側ではなく **ビルド対象のコード**で完結させる。
3 箇所を揃えてビルド→アップロード→再起動で反映する。

```
❌ NG: :root の font-family だけ変更 → Tailwind の font-sans ユーティリティが別フォントのまま
       Google Fonts のロードを変えず CSS だけ変更 → Web フォントが読み込まれない
...
### 教訓 15: サイト再起動は PAGES_WEBSITE_ID（PP API のサイト ID）で行う

> **deploy_site.py の自動再起動が「Site not found in PP API」で失敗した問題で確定。**

```
❌ NG: 再起動対象を powerpages.config.json の siteName（例: m365status）の
       部分一致で探す → PP API 登録名が "M365 Status Portal"（スペース入り）だと
       'm365status' が部分文字列にならず照合失敗 → 再起動されない
...
- `deploy_site.py` は `PAGES_WEBSITE_ID` → 正規化名前照合 の順で解決する。
