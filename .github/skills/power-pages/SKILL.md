...
```
Phase 0: Pre-Deploy Check（ビルド出力・テンプレート整合性・mspp_copy 検証・権限リンク検証）
...
Phase 4.5: Link Permissions（テーブル権限 → Authenticated Users 自動紐づけ）
  ⚠️ EDM では管理者でも Web ロール経由でないと 403 になる（バイパス不可）
  powerpagecomponent_powerpagecomponent 自己参照 N:N で全権限を紐づけ
Phase 5: Restart (Power Platform API でキャッシュクリア)
```

> **⚠️ 教訓: EDM では「管理者はバイパスされる」は嘘**
> ドキュメントでは System Administrator はテーブル権限をバイパスすると書かれているが、
> **実際にはポータル上では contact として認証されるため Web ロール紐づけが必須**。
> 管理者アカウントでも 403 になる。deploy_site.py は Phase 4.5 で自動的に紐づける。
