...
def build(scan: dict, blueprint: dict, decisions: dict) -> str:
    ...
    add("| # | 作業 | 手段 | 影響 |")
    add("| --- | --- | --- | --- |")
    add("| 1 | 不足している環境グループを作成 | `apply_environment_strategy.py --apply` | 追加のみ。既存環境に影響なし |")
    add("| 2 | 対象環境をマネージド環境化 | `set_managed_environment.py --apply` | Premium ライセンスが必要 |")
    add("| 3 | 環境をグループへ割り当て | 管理センター > 環境グループ > 環境の追加 | グループのルールを継承する |")
    add("| 4 | グループのルールを設定して発行 | 管理センター > 環境グループ > ルール > ルールの発行 | 環境側の設定がロックされる |")
    add("| 5 | ACP 推奨プロファイルを適用 | `apply_acp_profile.py --include-group --apply` | 許可リストを置換。事前に dry-run で差分確認 |")
    add("| 6 | ACP 専用モードへ切り替え | 管理センター > セキュリティ > データとプライバシー | クラシック DLP が評価対象外になる |")
    add("| 7 | Copilot クレジットを配分 | 管理センター > ライセンス > Copilot Credits | 環境ごとの上限 |")
    add("| 8 | 利用ガイドラインを SharePoint に公開 | `sharepoint` スキル | 読み取り専用の情報ページ |")
    add("| 9 | ガイドライン URL をウェルカム コンテンツへ設定 | 管理センター > 環境グループ > ルール | メーカーに表示される |")
    add("")
    ...
    add("- ACP 専用モード: 管理センターで混成モードへ戻すとクラシック DLP の評価が復活する（DLP は削除しない）。")
