...
## 15. Python スクリプトが `UnicodeEncodeError: 'cp932'`（絵文字の print）

**症状**: `register_mcp_client.py` 等が処理に成功しても、最後の `print("✅ ...")` で
`UnicodeEncodeError: 'cp932' codec can't encode character '\u2705'` を出して終了コード 1 になる。

**原因**: Windows の既定コンソールエンコーディング（cp932）が絵文字を扱えない。**登録処理自体は成功済み**。

**対処**: UTF-8 を有効にして再実行 or 状態確認する（副作用なし）。

```powershell
$env:PYTHONUTF8=1; python .github/skills/cowork/scripts/register_mcp_client.py --check
```

## 16. Graph でアプリ登録が `403 Authorization_RequestDenied`

**症状**: `setup_entra_oauth_graph.py`（Graph 経由の Entra アプリ作成）が
`403 Authorization_RequestDenied` / `Insufficient privileges` で失敗する。

**原因**: `auth_helper` の資格情報に **`Application.ReadWrite.All`（委任）**の管理者同意が無い。
App registration の作成には特権が要る。

**対処**:
- 管理者同意を付与してから再実行する（Entra 管理センター → アプリ → API のアクセス許可）。
