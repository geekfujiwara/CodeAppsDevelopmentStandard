...
> 自動化メモ: 上記のブラウザ操作は Playwright MCP（`browser_navigate` / `browser_click` /
> `browser_snapshot`）で自動化できる。対象は「ツール > 追加」メニューと対象サーバー詳細
> パネルの「確認 / Confirm」ボタン。API での代替（botcomponent 直接作成）は行わない。
