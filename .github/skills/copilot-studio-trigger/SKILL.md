...
**「メールに返信する (V3)」コネクタの Attachments 属性問題（2026-04-13 検証済み）:**
...
#### OneDrive for Business トリガーの例（検証済み）
...
### パターン 1: メール受信 → エージェント起動（検証済み実データ）

```python
# ★ 検証済みフロー定義（新しいメールが届いたとき (V3)）
...
### パターン 2: Teams メッセージ → エージェント起動（検証済み実データ）
...
```python
# ★ 検証済みフロー定義（チャネルに新しいメッセージが追加されたとき）
...
```python
# ★ 検証済みフロー定義（チャットに新しいメッセージが追加されたとき）
...
### パターン 3: スケジュール → エージェント起動（検証済み実データ）

スケジュールトリガーはコネクタ接続が不要。`shared_microsoftcopilotstudio` のみ必要。

```python
# ★ 検証済みフロー定義（Recurring Copilot Trigger）
...
### パターン 4: Dataverse レコード変更 → エージェント起動（検証済み実データ）

```python
# ★ 検証済みフロー定義（行が追加、変更、または削除された場合）
...
### パターン 5: SharePoint ファイル作成 → エージェント起動（検証済み実データ）

```python
# ★ 検証済みフロー定義（ファイルが作成されたとき (プロパティのみ)）
...
### パターン 6: OneDrive for Business ファイル作成 → エージェント起動（検証済み実データ）

```python
# ★ 検証済みフロー定義（ファイルが作成されたとき）
