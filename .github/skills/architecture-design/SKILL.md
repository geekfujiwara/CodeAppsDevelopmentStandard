...
| **AI Builder**     | 定型 AI タスク（分類・抽出・要約）、再利用可能な AI プロンプト、エージェントのツール化       | リアルタイム対話、複雑な推論チェーン                           |
...
ユーザー要件
    ...
    │                                     NO
    │                                     ↓
    ├─ データの閲覧・編集 UI が必要？ ──→ YES ──→ 【Code Apps or Canvas Apps】（§5 へ）
    │                                    NO
    │                                    ↓
...
## 5. Code Apps vs Canvas Apps の判断ポイント
...
└─ 複雑（カンバン + ガントチャート + インライン編集 + カスタムビジュアル）
    └─ → Code Apps 一択（Canvas Apps では実現困難）
```

| 判断ポイント                                         | Code Apps              | Canvas Apps          |
| ---------------------------------------------------- | ---------------------- | -------------------- |
| カスタム UI コンポーネント                           | ✅ 自由（React + OSS） | ❌ 制約あり          |
| ダーク/ライトモード                                  | ✅ Tailwind で容易     | △ 手動実装           |
| TanStack Table（ソート・フィルタ・ページネーション） | ✅ ネイティブ          | ❌ 非対応            |
| インライン編集                                       | ✅ InlineEditTable     | △ 手動実装           |
| ドラッグ&ドロップ（カンバン等）                      | ✅ dnd-kit             | ❌ 非対応            |
| ガントチャート・ツリー構造                           | ✅ 実装可能            | ❌ 困難              |
| モバイルネイティブ体験                               | △ レスポンシブ         | ✅ モバイル最適化    |
| 開発スピード（プロトタイプ）                         | △ コード記述必要       | ✅ ドラッグ&ドロップ |
| 市民開発者が保守可能                                 | ❌ TypeScript 必要     | ✅ ローコード        |
...
> **判断の理由**: Code Apps は保守性・拡張性・UI カスタマイズ性に優れる。
...
- [ ] **データ操作 UI が必要か？** → YES なら Code Apps / Canvas Apps を含む構成
