...
| **AI Builder**        | 定型 AI タスク（分類・抽出・要約）、再利用可能な AI プロンプト、エージェントのツール化（★ AI プロンプトを常に優先、プレビルトモデルは原則不使用） | リアルタイム対話、複雑な推論チェーン                           |
...
```
AI Builder には「プレビルトモデル」と「AI プロンプト（カスタムプロンプト）」の 2 種類がある。

■ プレビルトモデル（請求書処理、名刺読み取り、テキスト認識 等）
  - 事前学習済みのモデルを呼び出す方式
