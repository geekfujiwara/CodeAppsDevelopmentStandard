...
スクリプトは `sys.path` に `../../standard/scripts` を追加して import するため、コピー不要。
...
> **原則: 汎用プレースホルダーをそのまま使わない。** `generate_icon_png.py` の
> `draw_agent_icon()`（4 点スパークル）は**テーマ非依存の既定値**であり、ドメイン固有の
> コンポーネント（Cowork プラグイン・業務エージェント等）にそのまま流用しない。
> 必ず対象の **manifest.json の `name`/`description`、SKILL.md の目的説明、
> `accentColor`** を読み、そのコンポーネントが何を象徴すべきかを先に言語化してから
> モチーフ・配色を決める（例: 「合議」→ 吹き出し＋チェックマーク、「在庫管理」→ 箱＋グラフ 等）。
> コンテキストを読まずに既定のスパークルのまま登録するのは誤り。

### 提案方法

1. **対象のコンテキストを読む**: manifest.json（`name`/`description`/`accentColor`）や
   SKILL.md の目的説明から、コンポーネントの業務ドメイン・役割を特定する。
2. 特定したドメインに合ったアイコンを **3〜4 パターン** テキストで提案する
   （モチーフ・配色を具体的に：例「吹き出し3つ＋チェックマーク（合議・合意）」）。
3. 各パターンに簡単な説明を付けて提示する
4. ユーザーに選択してもらう
5. **選択されたパターン専用の `draw_<theme>_icon()` 関数を実装**（汎用関数の使い回しではなく、
   対象コンポーネント名を関数名に含めたスクリプトとして残す）→ Pillow で生成 →
   用途に応じた形式で API 登録・パッケージ同梱する

### アイコン設計ガイドライン

- **スタイル**: シンプルで視認性の高いデザイン。フラット or モダン
- **背景**: 角丸正方形（`rx="48" ry="48"`）。ブランドカラー推奨
- **モチーフ**: コンポーネントの目的を象徴するアイコン（テーマに合わせて選ぶ。例: スパークル・歯車・グラフ・チャットバブル等）
- **バリエーション例**:
