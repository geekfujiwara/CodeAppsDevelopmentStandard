...
```
✅ 正しい: テーブル geek_record、列 geek_description
❌ 間違い: テーブル geek_レコード、列 geek_説明
```
...
```python
# .env に必ず定義 — 全フェーズ（Phase 0〜7）で同じ値を使用
...
- `.github/skills/spec-builder/scripts/.venv` — markitdown 等
...
> **ルール**: 認証レコード（`~/.power-platform-cli/auth_record_{TENANT_ID}.json`）はリポジトリ外（ホームディレクトリ）に保存され、コミット対象にならない。何度もデバイスコード認証を求めるスクリプトは禁止。必ず `auth_helper` 経由で認証し、マシン全体で共有されるキャッシュを再利用すること。
...
| ルール              | 説明                                               | 例                                 |
| ------------------- | -------------------------------------------------- | ---------------------------------- |
| プレフィックス統一  | パブリッシャープレフィックスを全テーブル・列に統一 | `geek_record`, `geek_name`       |
...
Phase 4: Lookup リレーションシップ作成
  - geek_record → geek_category
  - geek_record → geek_asset
  - geek_record → systemuser (担当者)
  - geek_asset → geek_location
  - geek_asset → geek_assetcategory
  - geek_comment → geek_record
```
...
rels = api_get(f"/EntityDefinitions(LogicalName='geek_record')/ManyToOneRelationships")
for r in rels["value"]:
...
# Step 2: 列を削除
api_delete(f"/EntityDefinitions(LogicalName='geek_record')/Attributes(LogicalName='geek_reportedbyid')")
...
# 例: pac auth create --name AppManager --environment xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
...
await DataverseService.PostItem("geek_records", {
  geek_name: "サンプルレコード",
  geek_description: "説明文",
  geek_priority: 100000000, // 緊急
  geek_status: 100000000, // 新規
  "geek_categoryid@odata.bind": `/geek_categories(${categoryId})`,
...
export async function getRecords(): Promise<Geek_records[]> {
  const result = await Geek_recordsService.getAll({
    select: [
      "geek_recordid", "geek_name", "geek_status", "geek_priority", "createdon",
      "_geek_categoryid_value", "_geek_assignedtoid_value",
        ...
        "When_status_changes": {
            ...
            "inputs": {
                ...
                "parameters": {
                    ...
                    "subscriptionRequest/entityname": "geek_record",
...
```python
FLOW_DISPLAY_NAME = "レコードステータス変更通知"
...
headers = {
    ...
    "MSCRM.SolutionUniqueName": "SampleSolution",  # ← ソリューション自動追加
}
...
3. 「エージェント に名前をつける」ダイアログで以下を設定:
   ...
   - 「エージェント設定 (オプション)」を展開:
     - 言語: 日本語 (日本)
     - ソリューション: サンプルソリューション（SampleSolution）
     - スキーマ名: {PREFIX}_assistant
4. 「作成」をクリック
...
#### Step 0: 要件ヒアリング（コンサルタントモード）

**IT に詳しくないエンドユーザーへのプロの IT コンサルタントとして振る舞う。** 専門用語を使わず、業務課題を引き出す。

`AskUserQuestion` を使って以下を確認する（一問一答を避け、まとめて質問する）:
...
一言リクエストだけで全体像が掴みにくい場合は、以下を依頼する:

> 現在お使いの Excel ファイル・業務フロー図・画面イメージ・既存の帳票などがあれば共有してください。
> `/spec-builder` でドキュメントから要件を整理することもできます。

#### Step 1: ユーザー要件のヒアリング結果の整理

Step 0 のヒアリング結果をもとに以下を明確化する:
...
#### Step 1.5: 設計提案（Code Apps を含む場合）

Code Apps を含む構成の場合、テーブル設計と同時に以下を提案してユーザーに示す。

**デザインテンプレートの提案**

`code-apps` スキルの [`design-templates.md`](../../code-apps/references/design-templates.md) を読み込み、6 テンプレートの一覧表をユーザーに提示して選択を求める。

```
| # | テンプレート名      | 配色   | 印象                         |
|---|---------------------|--------|------------------------------|
| 1 | Ocean Blue          | 🔵青   | 爽やか・業務全般（デフォルト）|
| 2 | Indigo / Violet     | 🟣紫   | モダン・先進的               |
| 3 | Emerald / Teal      | 🟢緑   | ナチュラル・ヘルスケア       |
| 4 | Amber / Orange      | 🟠橙   | エネルギッシュ・製造・建設   |
| 5 | Rose / Pink         | 🌸ピンク| 親しみやすい・サービス業    |
| 6 | Slate Mono          | ⚫モノ | ミニマル・エンタープライズ   |
番号で選んでください（省略時: 1）
```

**近いサンプルの紹介**

`.github/skills/README.md` のサンプル一覧（geek-sales / geek-expense / geek-hr / geek-helpdesk / geek-inventory / geek-asset / geek-contract / geek-pm / geek-maintenance / geek-procurement / geek-kaizen / geek-event / geek-approval / geek-quote / geek-safety / geek-training / geek-quality）を確認し、ユーザーの要件に近いサンプルがあれば紹介する。

> 「○○管理に近いサンプルとして **geek-inventory**（在庫管理）があります。画面構成や Dataverse 接続パターンの参考にできます。」
...
| ------------------ | -------------------- | ------------------ | ---- |
| {prefix}\_record | {prefix}\_categoryid | {prefix}\_category | N:1  |
...
> **教訓**: 設計フェーズを省略してテーブルを作成すると、リレーションシップの漏れ（カテゴリ↔レコードの Lookup 未設定）、デモデータの漏れ（コメントテーブルにデータなし）、必要なマスタテーブルの漏れ（設備マスタ未作成）が発生する。設計レビューで防止できる。

#### Step 4: `spec/requirements.md` の生成（必須）

**設計承認直後に、以下の内容で `spec/requirements.md` を作成する。** ユーザーが意識しなくてもエージェントが自動生成する。
...
- **対象ユーザー**: {誰が使うか・人数}
- **利用環境**: {社内のみ / 外部公開}
- **連携システム**: {Teams / Outlook / 既存システム等}

## 機能一覧

| ID | 機能名 | 説明 | フェーズ | ステータス |
|---|---|---|---|---|
| F-001 | {機能名} | {説明} | Phase 6 | 🔴 未実装 |

> ステータス凡例: 🔴 未実装 / 🟡 実装中 / ✅ 実装済 / ⏭️ スコープ外

## 画面一覧

| 画面名 | ルート | 主要コンポーネント | 関連機能 |
|---|---|---|---|
| {画面名} | /{path} | ListTable / FormModal | F-001 |

## データモデル概要

| テーブル（日本語名） | スキーマ名 | 役割 |
|---|---|---|
| {テーブル名} | {prefix}_{name} | {役割} |
...
既存のドキュメント（Excel・仕様書等）がある場合は、`/spec-builder` で変換した内容をこのファイルに統合・補完する。
**このファイルは実装の進捗とともに更新し続ける。実装完了機能は ✅ に変更する。**

承認後、Phase 2 に進む。

### Phase 4〜7 の設計フェーズ（各フェーズ共通原則）

**AI Builder・Power Automate・Code Apps・Copilot Studio のいずれも、実装前に設計をユーザーに提示して承認を得る。**

> **依存関係に注意**: AI Builder → Power Automate → Code Apps の順で構築する。
> Power Automate フローは Code Apps から参照されるため、Code Apps より先に完成させる。
> AI Builder プロンプトは Power Automate フローから呼ばれるため、フローより先に完成させる。
...
#### Copilot Studio エージェント設計（Phase 7 開始前）
...
- [ ] AI Builder を呼ぶ場合は Phase 4 で AI プロンプトを先に作成済み
- [ ] **ユーザーにフロー設計を提示し、承認を得た**

### Code Apps UI 設計（Phase 6 開始前）

- [ ] `code-apps` スキルで画面構成・コンポーネントを設計済み
- [ ] Lookup 名前解決パターン（`_xxx_value` + `useMemo` Map）を設計に含めた
- [ ] Code Apps から呼ぶフローが Phase 5 で作成済みであること
