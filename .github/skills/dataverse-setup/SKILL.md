...
description: "Dataverse テーブル設計・構築・デモデータ投入。ソリューション作成からテーブル・リレーション・ローカライズ・データ投入・検証まで。Use when: Dataverse テーブル作成, テーブル設計, スキーマ設計, Lookup, Choice, リレーション, デモデータ, ソリューション, パブリッシャー, プレフィックス, setup_dataverse"
---
...
**コピー元:** `.github/skills/power-platform-standard/auth_helper.py`
...
同梱の [setup_dataverse.py](setup_dataverse.py) をプロジェクト用にカスタマイズして実行する。
...
[setup_dataverse.py](setup_dataverse.py)
