...
検出: テーブル名の直書き
  ...
  - OK（変更不要）: "geek_customerid@odata.bind"
...
動的化してはいけない例外（OData バインド）やシステム/カスタムテーブルの区別は
...
[`${import.meta.env.VITE_PUBLISHER_PREFIX ?? "fallback"}_records`]: { ... }
```
...
[ ] ルート .gitignore に .env / power.config.json / .power/ / src/generated/ が含まれている
    （ルート .gitignore はリポジトリ全体に適用される — サンプル個別の .gitignore は不要）

再利用性
[ ] VITE_PUBLISHER_PREFIX が vite-env.d.ts に宣言されている
[ ] src/config.ts に PUBLISHER_PREFIX エクスポートがある
[ ] カスタムテーブル名が services/*.ts で動的構築されている（const P = PUBLISHER_PREFIX）
[ ] Copilot Analytics 用カスタムテーブル（{prefix}_conversationsummaries 等）も動的化されている
[ ] システムテーブル（bots / conversationtranscripts / systemusers）はプレフィックスなしで正しい
[ ] lib/dataSourcesInfo.ts のカスタムテーブルエントリが動的プレフィックスを使っている
[ ] pages/*.tsx の列キー（key: `${P}_fieldname`）が動的化されている
[ ] OData バインド文字列（"geek_id@odata.bind"）は例外として残してよい
[ ] VITE_FEATURE_* による機能フラグが実装されている（オプション機能がある場合）
[ ] ナビゲーションが src/config.ts に固定記述されている（JSON env var 不使用）

ドキュメント
[ ] .env.example が samples/{name}/ 直下に存在し、取得元コメント付きで記載されている
[ ] README.md が samples/{name}/ 直下に存在し、以下を含む:
    - 含まれる機能（ページ一覧）
    - Dataverse テーブル一覧（{prefix} 形式）
    - カスタマイズポイント（変更箇所と説明）
    - セットアップ手順（コマンド付き）
    - 機能フラグ詳細
