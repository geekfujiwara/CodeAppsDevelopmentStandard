"""
Power Automate フローデプロイスクリプト — SharePoint ドキュメント自動レコード登録（サンプル）
Dataverse workflows テーブル方式

★ 本スクリプトはサンプル実装です。テーブル名・列名・アクションをプロジェクトに合わせて書き換えてください。
...
使い方:
  ...
  3. python .github/skills/power-automate/scripts/deploy_flow_sp_record.py
"""
...
FLOW_DISPLAY_NAME = "SharePoint ドキュメント自動レコード登録"
...
def build_clientdata(connections: dict, connref_dv: str, connref_sp: str) -> str:
    ...
    # odata.bind 式
    category_bind = (
        "/" + PREFIX + "_categories(@{first(outputs('List_Categories')?['body/value'])?['"
        + PREFIX + "_categoryid']})"
    )
        ...
        "actions": {
            ...
            "List_Categories": {
                ...
                "inputs": {
                    ...
                    "parameters": {
                        "entityName": f"{PREFIX}_categories",
...
def main():
    ...
    connref_dv = create_connection_reference(
        ...
        display_name="Dataverse (SP レコード登録)",
    ...
    connref_sp = create_connection_reference(
        ...
        display_name="SharePoint Online (SP レコード登録)",
    ...
    if not r2.ok:
        ...
        debug_path = "flow_sp_record_debug.json"
    ...
    print("  アクション: ファイル取得 → AI Builder 要約 → カテゴリ判定 → レコード作成")
    print()
